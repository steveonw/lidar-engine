...
_DEFAULT_ENGINES = [
    ...
    os.path.join(_HERE, "lidar_lenses_wave_v080_alpha5_1_patched (4).py"),
]
...
def parse_obj(text: str, color=(0.72, 0.72, 0.75), piece_id: int = 1000):
    ...
    for line in text.splitlines():
        line = line.split("#", 1)[0]   # drop inline comments before tokenizing
...
def run_scan(req: dict) -> dict:
    ...
    # Smart sampling: scout-then-fill concentrates the ray budget on the subject
    # instead of spraying it across empty space. Helps when the subject is small
    # in frame; ~matches uniform when it already fills the view. Not faster.
    smart = bool(req.get("smart_sampling", False))
    ...
    with tempfile.TemporaryDirectory() as out_dir:
        _extra = {"sampler": "scout_fill"} if smart else {}
        try:
            result = ENG.run_sensor_preset(
                scene, preset_name=preset, scene_name=scene_name,
                out_dir=out_dir, seed=seed, **_extra, **overrides,
            )
        except TypeError as _te:
            # Loaded engine predates the `sampler` hook — fall back to uniform.
            if smart and "sampler" in str(_te):
                smart = False
                result = ENG.run_sensor_preset(
                    scene, preset_name=preset, scene_name=scene_name,
    ...
    return {
        ...
        "stats": {
            ...
            "fast": fast,
            "smart_sampling": smart,
            "manual_camera": manual_camera,
            "camera_target": [round(x, 2) for x in diag.get("camera_target", [])],
        },
        "material_is_auto": (req.get("scene") == "upload"
                             and (req.get("material", "auto") in (None, "", "auto"))),
...
# ──────────────────────────────────────────────────────────────────────────
# Placement check — "did I place it right?" verification for generated scenes
#
# Reliable signals: canonical views (front/side/top, for the eye/AI to judge)
# and inventory + cross-type interpenetration (read from the true geometry).
# Floating is advisory only: plane/billboard geometry (stacked flat "logs")
# looks identical to genuinely floating objects, so the rule over-reports.
# ──────────────────────────────────────────────────────────────────────────
_CORNER_SIGNS = np.array([[sx, sy, sz] for sx in (-1, 1) for sy in (-1, 1)
                          for sz in (-1, 1)], float)


def _world_aabb(p):
    h = np.asarray(p.half_extents, float); R = np.asarray(p.rotation_matrix, float)
    c = (_CORNER_SIGNS * h) @ R.T + np.asarray(p.center, float)
    return c.min(0), c.max(0)


def _placement_report(prims):
    boxes = [_world_aabb(p) for p in prims]
    types = [getattr(p, "piece_type", "?") for p in prims]
    foot = [(b[1][0] - b[0][0]) * (b[1][2] - b[0][2]) for b in boxes]
    gi = int(np.argmax(foot)); gt = float(boxes[gi][1][1])
    inv = {}
    for t in types:
        inv[t] = inv.get(t, 0) + 1
    floaters = []
    for i, (mn, mx) in enumerate(boxes):
        if i == gi:
            continue
        b = float(mn[1]); cy = float(prims[i].center[1])
        if b <= gt + 0.12:
            continue
        sup = False
        for j, (m2, x2) in enumerate(boxes):
            if j == i or j == gi:
                continue
            if (mn[0] < x2[0] and mx[0] > m2[0] and mn[2] < x2[2] and mx[2] > m2[2]
                    and x2[1] >= b - 0.4 and m2[1] <= cy):
                sup = True; break
        if not sup:
            floaters.append({"index": i, "type": types[i], "gap_below": round(b - gt, 2),
                             "center": [round(float(x), 2) for x in prims[i].center]})
    floaters.sort(key=lambda f: -f["gap_below"])
    overlaps = []
    for i in range(len(boxes)):
        for j in range(i + 1, len(boxes)):
            if i == gi or j == gi or types[i] == types[j]:
                continue
            mn = np.maximum(boxes[i][0], boxes[j][0]); mx = np.minimum(boxes[i][1], boxes[j][1])
            ov = float(np.prod(np.clip(mx - mn, 0, None)))
            if ov <= 1e-9:
                continue
            va = float(np.prod(np.clip(boxes[i][1] - boxes[i][0], 0, None)))
            vb = float(np.prod(np.clip(boxes[j][1] - boxes[j][0], 0, None)))
            frac = ov / max(min(va, vb), 1e-9)
            if frac >= 0.25:
                overlaps.append({"a": i, "b": j, "types": [types[i], types[j]],
                                 "overlap_frac": round(float(frac), 2)})
    overlaps.sort(key=lambda o: -o["overlap_frac"])
    allmn = np.min([b[0] for b in boxes], 0); allmx = np.max([b[1] for b in boxes], 0)
    return {"object_count": len(prims),
            "bounds": {"min": [round(float(x), 2) for x in allmn],
                       "max": [round(float(x), 2) for x in allmx]},
            "inventory": inv, "interpenetrations": overlaps,
            "floaters_advisory": floaters}


def _scene_bounds_mm(scene):
    if isinstance(scene, list):
        boxes = [_world_aabb(p) for p in scene]
        return np.min([b[0] for b in boxes], 0), np.max([b[1] for b in boxes], 0)
    mns = [np.asarray(m.aabb_min, float) for m in scene.meshes]
    mxs = [np.asarray(m.aabb_max, float) for m in scene.meshes]
    return np.min(mns, 0), np.max(mxs, 0)


def _canonical_views(scene):
    mn, mx = _scene_bounds_mm(scene)
    center = (mn + mx) / 2.0
    radius = float(np.linalg.norm(mx - mn)) / 2.0 + 1e-3
    d = radius * 2.6; up_y = center[1] + radius * 0.25
    cams = {
        "front (−Z)": (np.array([center[0], up_y, center[2] + d]), np.array([0., 1, 0])),
        "side (+X)":  (np.array([center[0] + d, up_y, center[2]]), np.array([0., 1, 0])),
        "top (−Y)":   (np.array([center[0], center[1] + d, center[2] + 0.01]), np.array([0., 0, -1])),
    }
    cfg = ENG.get_sensor_preset("compact_diagnostic")
    imgs, labels = [], []
    for name, (pos, up) in cams.items():
        cam = ENG.Camera(position=pos, target=center.astype(float), up=up,
                         fov_deg=50, width=240, height=170, lens="pinhole")
        b = ENG.fire_burst(cam, scene, 240 * 170 * 2, 1)
        ch = ENG.compute_wave_channels(b, scene)
        cl = ENG.classify_to_label_array(ch, cfg)
        im, _ = ENG._render_panel("shaded", b, ch, scene, cfg, classified=cl)
        imgs.append(im); labels.append(name)
    return ENG.composite_grid(imgs, labels, cols=3)


def run_placement_check(req: dict) -> dict:
    scene, scene_name, info = build_scene(req)
    is_prims = isinstance(scene, list)
    sheet = _canonical_views(scene)
    buf = io.BytesIO(); sheet.save(buf, "PNG")
    views = "data:image/png;base64," + base64.b64encode(buf.getvalue()).decode()
    report = _placement_report(scene) if is_prims else None
    return {"ok": True, "scene_info": info, "scene_name": scene_name,
            "is_primitive": is_prims, "views": views, "report": report}


# ──────────────────────────────────────────────────────────────────────────
# Multi-view fused point cloud → downloadable interactive HTML
#
# Scans the scene from 5 canonical viewpoints and fuses the hits. The clouds
# auto-register: every hit is already in world coordinates and the camera poses
# are exact, so fusing is just concatenation — no ICP/alignment. Points are
# coloured by which view saw them, so coverage gaps read as holes.
# ──────────────────────────────────────────────────────────────────────────
_PC_PALETTE = [("front", (1.0, 0.40, 0.30)), ("back", (1.0, 0.80, 0.30)),
               ("left", (0.30, 1.0, 0.50)), ("right", (0.30, 0.70, 1.0)),
               ("top", (0.80, 0.40, 1.0))]


def _build_pointcloud_html(P, C, legend):
    c = P.mean(0); pr = P - c
    ext = pr.max(0) - pr.min(0)
    rad = round(float(np.linalg.norm(ext)) / 2.0 * 1.8 + 1.0, 2)
    flat = []
    for p, cc in zip(pr, C):
        flat += [round(float(p[0]), 3), round(float(p[1]), 3), round(float(p[2]), 3),
                 round(float(cc[0]), 3), round(float(cc[1]), 3), round(float(cc[2]), 3)]
    data = json.dumps(flat)
    leg = "".join('<span class=k style="background:rgb(%d,%d,%d)"></span>%s '
                  % (int(r * 255), int(g * 255), int(b * 255), name)
                  for name, (r, g, b) in legend)
    tmpl = """<!DOCTYPE html><html><head><meta charset="utf-8"><title>fused point cloud</title>
<style>html,body{margin:0;height:100%;background:#0a0e14;overflow:hidden;font-family:sans-serif}
#hud{position:fixed;top:10px;left:12px;color:#8fa;font-size:13px;line-height:1.7}
.k{display:inline-block;width:10px;height:10px;margin-right:5px;border-radius:2px}</style>
<script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script></head>
<body><div id="hud"><b>fused point cloud</b> &mdash; __N__ points, 5 views<br>__LEGEND__<br>drag to orbit &middot; scroll to zoom</div>
<script>
var D=__DATA__;
var scene=new THREE.Scene();
var cam=new THREE.PerspectiveCamera(55,window.innerWidth/window.innerHeight,0.05,2000);
var r=new THREE.WebGLRenderer({antialias:true});r.setSize(window.innerWidth,window.innerHeight);
r.setClearColor(0x0a0e14);document.body.appendChild(r.domElement);
var n=D.length/6;var pos=new Float32Array(n*3),col=new Float32Array(n*3);
for(var i=0;i<n;i++){pos[i*3]=D[i*6];pos[i*3+1]=D[i*6+1];pos[i*3+2]=D[i*6+2];col[i*3]=D[i*6+3];col[i*3+1]=D[i*6+4];col[i*3+2]=D[i*6+5];}
...
scene.add(new THREE.Points(g,new THREE.PointsMaterial({size:Math.max(0.02,__RAD__*0.004),vertexColors:true})));
var th=0.7,ph=1.1,rad=__RAD__,tgt=new THREE.Vector3(0,0,0),down=false,px=0,py=0;
function upd(){cam.position.set(tgt.x+rad*Math.sin(ph)*Math.cos(th),tgt.y+rad*Math.cos(ph),tgt.z+rad*Math.sin(ph)*Math.sin(th));cam.lookAt(tgt);}
window.addEventListener('mousedown',function(e){down=true;px=e.clientX;py=e.clientY;});
window.addEventListener('mouseup',function(){down=false;});
window.addEventListener('mousemove',function(e){if(!down)return;th+=(e.clientX-px)*0.005;ph=Math.max(0.05,Math.min(3.09,ph-(e.clientY-py)*0.005));px=e.clientX;py=e.clientY;upd();});
window.addEventListener('wheel',function(e){rad=Math.max(0.5,Math.min(__RAD__*5,rad*(1+e.deltaY*0.001)));upd();});
...
upd();(function loop(){requestAnimationFrame(loop);th+=0.0008;upd();r.render(scene,cam);})();
</script></body></html>"""
    return (tmpl.replace("__DATA__", data).replace("__N__", str(len(P)))
            .replace("__LEGEND__", leg).replace("__RAD__", str(rad)))


def run_pointcloud(req: dict) -> dict:
    scene, scene_name, info = build_scene(req)
    mn, mx = _scene_bounds_mm(scene)
    center = (mn + mx) / 2.0
    radius = float(np.linalg.norm(mx - mn)) / 2.0 + 1e-3
    d = radius * 2.6; eye_y = center[1] + radius * 0.25
    views = {
        "front": (np.array([center[0], eye_y, center[2] + d]), np.array([0., 1, 0])),
        "back":  (np.array([center[0], eye_y, center[2] - d]), np.array([0., 1, 0])),
        "left":  (np.array([center[0] - d, eye_y, center[2]]), np.array([0., 1, 0])),
        "right": (np.array([center[0] + d, eye_y, center[2]]), np.array([0., 1, 0])),
        "top":   (np.array([center[0], center[1] + d, center[2] + 0.01]), np.array([0., 0, -1])),
    }
    budget = 12000
    # Honor the "Smart sampling" checkbox: on → concentrate the budget on the
    # subject (scout-then-fill), so a thin model fills in instead of wasting rays
    # on empty space; off → plain uniform. Falls back to uniform if the engine
    # predates the sampler.
    smart = bool(req.get("smart_sampling", False))
    _scout = getattr(ENG, "scout_then_fill", None) if smart else None
    pts, cols = [], []
    for name, col in _PC_PALETTE:
        pos, up = views[name]
        cam = ENG.Camera(position=pos, target=center.astype(float), up=up,
                         fov_deg=50, width=200, height=150, lens="pinhole")
        b = (_scout(cam, scene, budget, 1) if _scout is not None
             else ENG.fire_burst(cam, scene, budget, 1))
        hit = b.depths < ENG.INF
        wp = b.origins[hit] + b.dirs[hit] * b.depths[hit][:, None]
        if len(wp):
            pts.append(wp); cols.append(np.tile(col, (len(wp), 1)))
    if not pts:
        raise ValueError("scan returned no points (scene empty or out of frame)")
    P = np.concatenate(pts).astype(np.float32)
    C = np.concatenate(cols).astype(np.float32)
    html = _build_pointcloud_html(P, C, _PC_PALETTE)
    return {"ok": True, "filename": scene_name + "_pointcloud.html",
            "points": int(len(P)),
            "sampler": ("scout_fill" if _scout is not None
                        else ("uniform (engine lacks scout-fill)" if smart else "uniform")),
...
<div class="wrap">
  <div class="panel">
    ...
    <label class="chk"><input type="checkbox" id="fast" checked> Fast preview (cheap framing, single burst)</label>
    <label class="chk"><input type="checkbox" id="smartSampling"> Smart sampling (scout-then-fill — concentrates rays on the subject; applies to scans <i>and</i> the point cloud; best when it's small in frame)</label>
    <button id="placement" style="width:100%;margin-top:10px;padding:9px;background:#243044;border:1px solid #3a4a66;border-radius:7px;color:#cfe0f5;cursor:pointer">Check placement (canonical views + geometry report)</button>
    <button id="pointcloud" style="width:100%;margin-top:8px;padding:9px;background:#243044;border:1px solid #3a4a66;border-radius:7px;color:#cfe0f5;cursor:pointer">Build point cloud (download interactive HTML)</button>
...
function fileToB64(f) {
  // Use readAsDataURL and strip the "data:...;base64," prefix. The old
  // String.fromCharCode(...new Uint8Array(...)) approach spreads every byte as
  // a call argument and throws RangeError ("Maximum call stack size exceeded")
  // on files larger than ~64 KB — i.e. essentially every real mesh or engine.
...
async function run() {
  ...
  const req = {
    ...
    fast: $("fast").checked,
    smart_sampling: $("smartSampling").checked,
...
async function checkPlacement() {
  if (!engineLoaded) { alert("Load an engine first."); return; }
  const req = { scene, normalize: $("normalize").checked, simplify: $("simplify").checked,
                max_tris: $("maxTris").value, material: $("material").value };
  if (scene === "upload") {
    const f = $("file").files[0];
    if (!f) { alert("Choose a .stl or .obj file first."); return; }
    req.filename = f.name; req.data_b64 = await fileToB64(f);
  }
  $("placement").disabled = true;
  $("out").innerHTML = '<div class="empty"><span class="spin"></span>Rendering canonical views…</div>';
  try {
    const r = await fetch("/placement_check", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(req)});
    const data = await r.json();
    if (!data.ok) { render_error(data.error || "unknown error"); } else { renderPlacement(data); }
  } catch (err) { render_error(String(err)); }
  $("placement").disabled = false;
}
function renderPlacement(d) {
  let h = '<h3 style="margin:0 0 10px">Placement check</h3>';
  h += `<img src="${d.views}" style="width:100%;border-radius:8px;border:1px solid #2a3550">`;
  if (d.report) {
    const r = d.report;
    h += `<p style="margin:10px 0 4px;color:#9fb">objects: ${r.object_count} · bounds ${JSON.stringify(r.bounds.min)} .. ${JSON.stringify(r.bounds.max)}</p>`;
    h += `<p style="margin:4px 0"><b>inventory:</b> ${Object.entries(r.inventory).map(([k,v])=>k+":"+v).join("  ")}</p>`;
    h += `<p style="margin:4px 0"><b>interpenetrations:</b> ${r.interpenetrations.length ? r.interpenetrations.map(o=>`#${o.a} ${o.types[0]}∩#${o.b} ${o.types[1]} (${Math.round(o.overlap_frac*100)}%)`).join("; ") : "none"}</p>`;
    h += `<p style="margin:4px 0;color:#c98"><b>floating (advisory):</b> ${r.floaters_advisory.length} flagged — unreliable on plane/billboard geometry; trust the views + inventory</p>`;
  } else {
    h += `<p style="margin:10px 0;color:#9ab">Uploaded mesh: canonical views only (per-object geometry report needs a primitive scene).</p>`;
  }
  $("out").innerHTML = '<div style="padding:6px">' + h + '</div>';
}
$("placement").addEventListener("click", checkPlacement);
async function buildPointcloud() {
  ...
  $("pointcloud").disabled = true;
  $("out").innerHTML = '<div class="empty"><span class="spin"></span>Scanning 5 views and fusing the cloud…</div>';
  try {
    const r = await fetch("/pointcloud", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(req)});
    const data = await r.json();
    if (!data.ok) { render_error(data.error || "unknown error"); }
    else {
      const html = atob(data.html_b64);
      ...
      $("out").innerHTML = '<div style="padding:12px;color:#9fb">Downloaded <b>' + data.filename + '</b> — ' + data.points + ' points fused from 5 views · sampler: <b>' + data.sampler + '</b>. Open it in a browser to orbit the cloud.</div>';
    }
  } catch (err) { render_error(String(err)); }
  $("pointcloud").disabled = false;
}
$("pointcloud").addEventListener("click", buildPointcloud);
...
function render(d) {
  ...
  // advisory banners
  const sb = d.stats || {}, dsb = sb.depth_stats || {};
  if (dsb.coverage != null && dsb.coverage < 0.35) {
    h += `<div class="card" style="background:#2a230f;border-color:#6b5a1e;color:#e8d48a">`
       + `⚠ Low coverage (${(dsb.coverage*100).toFixed(1)}%) — most of the frame is background. `
       + `The object may be far, off-centre, or the manual camera mis-aimed. Try moving the camera closer, `
       + `widening FOV, re-targeting the object centre, or turning off manual camera to auto-frame.</div>`;
  }
  if (d.material_is_auto) {
    h += `<div class="card" style="background:#162033;border-color:#2c3a55;color:#9fb6d8">`
       + `ℹ Material = <b>auto</b>: the mesh has no assigned material, so the acoustic / ultrasonic / `
       + `polarization channels all fall back to the same neutral prior and carry no material information `
       + `(they'll read identically). Pick a material to make them meaningful; for auto uploads, trust the `
       + `geometry channels (depth, edges, silhouette, coverage).</div>`;
  }
  ...
  h += `<div class="k">mode</div><div class="v">${s.manual_camera ? "manual camera capture" : (s.fast ? "fast preview (cheap framing, single burst)" : "high quality (auto-framed, stacked)")}</div>`;
  let rt = `${s.total_runtime_seconds}s`;
  const parts = [];
  if (s.render_runtime_seconds != null) parts.push(`render ${s.render_runtime_seconds}s`);
  if (s.report_runtime_seconds != null) parts.push(`reports ${s.report_runtime_seconds}s`);
  if (s.autoframe_runtime_seconds != null) parts.push(`auto-frame ${s.autoframe_runtime_seconds}s`);
  if (parts.length) rt += ` <span style="color:#7f8da3">(${parts.join(", ")})</span>`;
  h += `<div class="k">runtime</div><div class="v">${rt}</div>`;
  h += '</div></div>';
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _content_length(self) -> int:
        try:
            ...
            return 0
        return max(0, n)  # a negative Content-Length would make read(n) read to EOF
    ...
    def do_POST(self):
        ...
        try:
            path = urlparse(self.path).path
            if path not in ("/scan", "/placement_check", "/pointcloud", "/load_engine", "/upload_engine"):
                self._send(404, json.dumps({"ok": False, "error": "not found"}))
                return

            # Engine-loading endpoints are RCE by design — refuse them unless
            # explicitly enabled (always on for a loopback bind).
            if path in ("/load_engine", "/upload_engine") and not ALLOW_ENGINE_LOAD:
            ...
            # gigabytes just by claiming a huge Content-Length. For an oversized
            # body we close the connection rather than draining it — draining a
            # huge slow upload would tie up the worker thread.
            n = self._content_length()
            if n > MAX_BODY_BYTES:
            ...
            with _LOCK:
                ...
                elif path == "/placement_check":
                    self._send(200, json.dumps(run_placement_check(req)))
                elif path == "/pointcloud":
                    self._send(200, json.dumps(run_pointcloud(req)))
                elif path == "/load_engine":
