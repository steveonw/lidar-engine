...
def run_sensor_preset(prims: List[Primitive],
    ...
    if not bool(cfg.get("wave", True)):
        burst = fire_burst(
            cam, prims, n_samples=n_samples, seed=seed,
            beam_profile=cfg.get("beam_profile", "gaussian"),
            beam_width=float(cfg.get("beam_width", 0.35)),
            min_ray_weight=float(cfg.get("min_ray_weight", 0.05)),
        )
        last_burst = burst
        imgs, labels = [], []
        ...
        for k in range(stack):
            b = pilot if k == 0 else fire_burst(
                cam, prims, n_samples=n_samples, seed=seed + k,
                beam_profile=cfg.get("beam_profile", "gaussian"),
                beam_width=float(cfg.get("beam_width", 0.35)),
                min_ray_weight=float(cfg.get("min_ray_weight", 0.05)),
            )
