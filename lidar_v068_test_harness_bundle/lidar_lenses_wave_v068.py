"""
lidar_lenses_wave.py — standalone multimodal ray/wave sensor sandbox (v0.6.8)
...
def effective_edge_threshold_from_channels(channels: Dict[str, np.ndarray],
    ...
    v0.6.8 adds an upper clamp (`edge_anti_max`, default 0.95).  Without
...
def classify_pixels(channels: Dict[str, np.ndarray],
    ...
    # v0.6.8: use the shared continuous edge score helper so binary geom_edge
...
def _build_material_target_board_scene() -> List[Primitive]:
    ...
    v0.6.8: The earlier notebook material_targets scene could under-expose
...
# ─────────────────────────────────────────────────────────────────────────
#  GEOM_EDGE AS FIRST-CLASS STRUCTURE / BOUNDARY LAYER (v0.6.8)
# ─────────────────────────────────────────────────────────────────────────
...
def run_sensor_preset(prims: List[Primitive],
    ...
    return {
        ...
        "material_label_agreement": material_label_agreement_report(
            last_burst, channels, prims, classifier_kwargs_from_config(cfg),
            label_arr=classified[0] if classified is not None else None
        ) if channels is not None else [],
        "material_core_agreement": material_core_agreement_report(
            last_burst, channels, prims, classifier_kwargs_from_config(cfg),
            label_arr=classified[0] if classified is not None else None
        ) if channels is not None else [],
        "material_filled_agreement": material_filled_agreement_report(
            last_burst, channels, prims, classifier_kwargs_from_config(cfg),
            radius=int(cfg.get("edge_fill_radius", 1)),
            min_fraction=float(cfg.get("edge_fill_min_fraction", 0.65)),
            fine_priority=bool(cfg.get("edge_fill_fine_priority", False)),
        ...
        "boundary_adjacency": boundary_adjacency_report(
            channels, classifier_kwargs_from_config(cfg),
            label_arr=classified[0] if classified is not None else None
        ) if channels is not None else [],
        "structure_density": structure_density_report(
            channels, classifier_kwargs_from_config(cfg),
            label_arr=classified[0] if classified is not None else None,
