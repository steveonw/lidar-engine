"""
lidar_lenses_wave.py — standalone multimodal ray/wave sensor sandbox (v0.6.11)
...
def compute_wave_channels(burst: Burst,
    ...
    Args:
        ...
        carrier_mode: "x" for the legacy x-only carrier, "ensemble" for
            median-combined rotated carriers. v0.6.11 defaults the ensemble
        ...
        edge_score_mode: "geom_fused" (default) uses anti-wave evidence gated
            ...
            v0.6.11 pairs geom_fused with a lower default edge floor because
    ...
    if carrier_mode == "x":
        ...
        if carrier_angles is None:
            # v0.6.11: golden-angle four-basis ensemble.  The older cross
    ...
    # exist.  v0.6.11 keeps raw anti-wave evidence separate from the geometry-
...
def edge_score_from_channels(channels: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    v0.6.11 prefers `channels["edge_score"]`, which may be the geometry-fused
...
def effective_edge_threshold_from_channels(channels: Dict[str, np.ndarray],
    ...
    v0.6.11 adds an upper clamp (`edge_anti_max`, default 0.95).  Without
...
def classify_pixels(channels: Dict[str, np.ndarray],
    ...
    Adaptive edge threshold:
        ...
        adaptive_edge_percentile) over hit pixels).  In v0.6.11 `edge_score` is geometry-fused by default and
    ...
    # v0.6.11: use the shared continuous edge score helper so binary geom_edge
...
def _build_material_target_board_scene() -> List[Primitive]:
    ...
    v0.6.11: The earlier notebook material_targets scene could under-expose
...
# ─────────────────────────────────────────────────────────────────────────
#  GEOM_EDGE AS FIRST-CLASS STRUCTURE / BOUNDARY LAYER (v0.6.11)
# ─────────────────────────────────────────────────────────────────────────
