"""
lidar_lenses_wave.py — standalone multimodal ray/wave sensor sandbox (v0.6.1)
...
Capabilities:
  - v0.6.1 classifier tuning: adaptive anti-edge threshold by scene percentile
...
def classify_pixels(channels: Dict[str, np.ndarray],
    ...
    # v0.6.1: adaptive anti-edge threshold.
    ...
    if U_i is not None and A_tex is not None:
        ...
        # SOFT_MATERIAL: lopsided spectrum (audible > ultrasonic) but not
        # extreme enough to be foliage.  Intensity floor keeps sky misses
        # and pure foliage out.  Skin/fabric land in the 0.60-0.70 texture
        # band; logs/wood at 0.567 and below sit below the threshold.
        soft_material = (has_hit & (A_tex > soft_material_texture_min) &
                          (A_tex <= foliage_texture_min) &
...
# ═════════════════════════════════════════════════════════════════════════
# v0.6.1 SENSOR PRESETS — consistent scene test packs
# ─────────────────────────────────────────────────────────────────────────
...
def scene_bounds(prims: List[Primitive]) -> Dict[str, np.ndarray]:
    ...
    v0.6.1 note: this uses per-axis extents instead of max-half-extent as a
