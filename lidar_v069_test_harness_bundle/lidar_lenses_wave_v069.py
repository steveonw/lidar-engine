"""
lidar_lenses_wave.py — standalone multimodal ray/wave sensor sandbox (v0.6.9)
...
Capabilities:
  - v0.6.9 stability: preset-specific contact sheets, runtime split diagnostics, and edge-case contracts
...
def compute_wave_channels(burst: Burst,
    ...
    # Continuous structure evidence.  With sound disabled this falls back to
    # optical anti-coherence; once sound_anti exists below we update it to the
    # cross-modal minimum.
    out["edge_score"] = out["light_anti"].copy()
    ...
    if include_sound and prims is not None:
        ...
        out["edge_score"] = np.minimum(out["light_anti"], out["sound_anti"])
...
def edge_score_from_channels(channels: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    The binary geom_edge label is thresholded from this score, but exposing the
    score separately makes edge diagnostics/presets easier to tune.
    """
...
def effective_edge_threshold_from_channels(channels: Dict[str, np.ndarray],
    ...
    v0.6.9 adds an upper clamp (`edge_anti_max`, default 0.95).  Without
...
def classify_pixels(channels: Dict[str, np.ndarray],
    ...
    # v0.6.9: use the shared continuous edge score helper so binary geom_edge
...
def _build_material_target_board_scene() -> List[Primitive]:
    ...
    v0.6.9: The earlier notebook material_targets scene could under-expose
...
SENSOR_PRESETS: Dict[str, Dict] = {
    ...
    "indoor_structure": {
        ...
        "panels": [
            ...
            "geom_edge_overlay", "geom_edge_mask",
    ...
    "compact_diagnostic": {
        ...
        "panels": [
            "shaded", "depth", "light_anti", "sound_anti",
            "edge_score", "edge_confidence", "geom_edge_overlay", "classification",
        ],
    },
    "edge_debug": {
        ...
        "classification_style": "indoor_sparse",
        "panels": [
            "shaded", "edge_score", "edge_confidence", "geom_edge_mask",
...
# ─────────────────────────────────────────────────────────────────────────
#  GEOM_EDGE AS FIRST-CLASS STRUCTURE / BOUNDARY LAYER (v0.6.9)
# ─────────────────────────────────────────────────────────────────────────
