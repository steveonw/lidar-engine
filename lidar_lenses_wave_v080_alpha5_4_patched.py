"""
lidar_lenses_wave.py — standalone multimodal ray/wave sensor sandbox (v0.8.0-alpha5.1 BeamEvidenceLite)

Single-file NumPy + PIL simulator built from a clean analytic raycaster,
extended into a synthetic sensing laboratory.  Only dependencies: NumPy + Pillow. No Blender/Unity/ROS simulator required.

Capabilities:
  - v0.8.0-alpha5.1 BeamEvidenceLite: weighted beam returns with near/far/spread/mix summaries
  - v0.7.0 release candidate promoted to stable public experimental engine
  - v0.6.11 stability: preset-specific contact sheets, runtime split diagnostics, and edge-case contracts
  - v0.6.8 edge threshold guardrail with edge_score / edge_confidence diagnostics
  ...
  - Multi-category opinionated classifier with optional material-label mode
...
class Burst:
    ...
    ray_weights: Optional[np.ndarray] = None
    coverage: float = 0.0
    unique_pieces: int = 0
...
def _confidence_heatmap(conf_arr: np.ndarray,
                        ...
                        smooth: bool = False,
                        absolute: bool = False) -> Image.Image:
    """Render a confidence array as a viridis-like heatmap.

    By default the array is normalized by its frame max — correct for
    relative/evidence channels, but it makes a *uniform* field saturate to full
    scale (a dead constant channel then looks maxed-out). Pass absolute=True for
    channels already in [0,1] whose magnitude is meaningful (material intensity,
    softness, texture, polarization) so 0.3 renders dim and 0.9 renders bright.
    """
    if absolute:
        cf = np.clip(conf_arr, 0.0, 1.0)
    else:
        H_max = conf_arr.max()
        if H_max < EPS:
...
class Mesh:
    ...
    def __post_init__(self):
        ...
        # Validate up front so malformed/empty meshes fail with a clear message
        # instead of crashing later in .min(axis=0) or deep inside raycasting.
        if self.vertices.ndim != 2 or self.vertices.shape[1] != 3:
            raise ValueError(f"Mesh vertices must be (N,3); got {self.vertices.shape}")
        if self.faces.ndim != 2 or self.faces.shape[1] != 3:
            raise ValueError(f"Mesh faces must be (M,3); got {self.faces.shape}")
        ...
        if self.faces.min() < 0 or self.faces.max() >= len(self.vertices):
            raise ValueError(
                f"Mesh face index out of range: verts={len(self.vertices)}, "
                f"face range {int(self.faces.min())}..{int(self.faces.max())}")
...
def _load_stl_binary(buf: bytes, color, piece_id) -> Mesh:
    if len(buf) < 4:
        raise ValueError("binary STL too short: missing triangle count")
    n_tri = struct.unpack("<I", buf[:4])[0]
    # Guard against a malformed/hostile header claiming a huge triangle count:
    # the body must actually be long enough to hold them (50 bytes each), which
    # makes the giant-allocation DoS impossible from a small file. The absolute
    # cap is a secondary backstop for legitimate-but-absurd files.
    expected = 4 + n_tri * 50
    if len(buf) < expected:
        raise ValueError(
            f"binary STL truncated: header claims {n_tri} triangles "
            f"({expected} bytes) but body is {len(buf)} bytes")
    MAX_TRIS = 20_000_000
    if n_tri > MAX_TRIS:
        raise ValueError(f"binary STL triangle count {n_tri} exceeds cap {MAX_TRIS}")
...
def beam_ray_weights(pixels: np.ndarray,
                     beam_profile: str = "gaussian",
                     beam_width: float = 0.35,
                     min_weight: float = 0.05) -> np.ndarray:
    """Return one lightweight beam-shape weight per ray.

    BeamReturnLite intentionally does not simulate a full waveform.  This
    function only says how strongly each sub-pixel ray contributes to its
    pixel/beam return summary.

    beam_profile:
      "flat" / "uniform" / None  -> all rays weight 1.0
      "gaussian"                 -> center of pixel/beam is strongest,
                                    edges are softer
    beam_width is in pixel units.  A value around 0.30-0.40 gives a visible
    beam profile without erasing edge samples.
    """
    if pixels is None or len(pixels) == 0:
        return np.zeros(0, dtype=np.float64)
    mode = str(beam_profile or "flat").lower()
    if mode in ("flat", "uniform", "none", "off"):
        return np.ones(len(pixels), dtype=np.float64)

    frac_x = pixels[:, 0] - np.floor(pixels[:, 0]) - 0.5
    frac_y = pixels[:, 1] - np.floor(pixels[:, 1]) - 0.5
    r2 = frac_x * frac_x + frac_y * frac_y

    if mode in ("gaussian", "gauss"):
        sigma2 = max(float(beam_width), 1e-6) ** 2
        w = np.exp(-0.5 * r2 / sigma2)
    else:
        raise ValueError(f"unknown beam_profile: {beam_profile!r}. Use 'gaussian' or 'flat'.")
    return np.clip(w, float(min_weight), 1.0).astype(np.float64)


def fire_burst(cam: Camera, prims: List[Primitive], n_samples: int, seed: int,
               beam_profile: str = "gaussian",
               beam_width: float = 0.35,
               min_ray_weight: float = 0.05) -> Burst:
    """Fire a burst using the camera's lens type.

    v0.8.0-alpha5.1 keeps per-ray weights for BeamReturnLite.  Existing callers
    remain valid; by default this uses a simple Gaussian beam footprint.
    Pass beam_profile="flat" for v0.7-like uniform ray weights.
    """
    rng = np.random.default_rng(seed)
    gen = LENS_REGISTRY.get(cam.lens, rays_pinhole)
    origins, dirs, pixels = gen(cam, n_samples, rng)
    ray_weights = beam_ray_weights(
        pixels,
        beam_profile=beam_profile,
        beam_width=beam_width,
        min_weight=min_ray_weight,
    )
    ...
    return Burst(
        ...
        normals=normal, piece_ids=pid, ray_weights=ray_weights,
...
def add_sensor_noise(burst: Burst,
    ...
    return Burst(
        ...
        ray_weights=getattr(burst, "ray_weights", None),
...
def stack_channels(channel_dicts: List[Dict[str, np.ndarray]]
    ...
    total_return_weight = (sum(ch.get("return_weight_sum", ch.get("hit_count", 0))
                               for ch in channel_dicts)
                           if has_hits else None)
    total_return_expected = (sum(ch.get("return_weight_expected", ch.get("hit_count", 0))
                                 for ch in channel_dicts)
                             if has_hits else None)
    weighted_return_keys = {
        "return_depth_mean", "return_depth_near", "return_depth_far",
        "return_depth_spread", "return_mix_score_raw", "return_mix_score",
        "return_expected_spread", "return_depth_spread_excess",
        "beam_split_score", "beam_front_depth", "beam_back_depth",
        "beam_front_strength", "beam_back_strength", "beam_mode_gap",
        "beam_mode_confidence", "beam_coherence",
        "beam_mode_gap_ratio", "beam_back_ratio",
    }
    for k in keys:
        if k == "hit_count":
            out[k] = total_hits
        elif k == "return_weight_sum":
            out[k] = total_return_weight
        elif k == "return_weight_expected":
            out[k] = total_return_expected
        elif k == "return_coverage" and total_return_weight is not None and total_return_expected is not None:
            out[k] = np.where(total_return_expected > 0,
                               np.clip(total_return_weight / np.maximum(total_return_expected, 1e-9), 0, 1),
                               0.0)
        elif k == "return_valid_stats":
            # Treat validity as a support fraction across stacked bursts.
            ...
            out[k] = np.where(total_hits > 0,
                               ...
                               0.0)
        elif k in weighted_return_keys and total_return_weight is not None:
            weighted = sum(ch[k] * ch.get("return_weight_sum", ch.get("hit_count", 0))
                            for ch in channel_dicts)
            out[k] = np.where(total_return_weight > 0,
                               weighted / np.maximum(total_return_weight, 1e-9),
                               0.0)
        else:
            out[k] = sum(ch[k] for ch in channel_dicts) / n
    return out
...
def compute_wave_channels(burst: Burst,
                          ...
                          edge_score_mode: str = "geom_fused",
                          edge_fusion_mode: str = "depth_grad_mul",
                          include_sound: bool = True,
                          include_ultrasonic: bool = False,
                          include_polarization: bool = False,
                          attenuation_per_meter: float = 0.0,
                          return_mix_gain: float = 1.0,
                          return_q_near: float = 15.0,
                          return_q_far: float = 85.0,
                          return_expected_spread_rel: float = 0.01,
                          return_expected_spread_grad: float = 0.10,
                          min_return_coverage: float = 0.25,
                          min_return_hit_count: int = 4,
                          beam_split_min_weight_frac: float = 0.10,
                          beam_split_min_abs_gap: float = 0.10,
                          beam_split_confidence_gain: float = 0.50,
                          beam_coherence_mix_gain: float = 6.0,
                          beam_coherence_split_gain: float = 0.80,
    ...
    Args:
        ...
        carrier_mode: "x" for the legacy x-only carrier, "ensemble" for
            median-combined rotated carriers. v0.7.0 defaults the ensemble
            to a golden-angle 4-basis schedule to reduce directional striping.
        carrier_angles: optional list of carrier angles in radians used when
            carrier_mode="ensemble". Defaults to a golden-angle 4-basis set.
        edge_score_mode: "geom_fused" (default) uses anti-wave evidence gated
            by geometric support; "raw_anti" uses raw min(light_anti,sound_anti).
            v0.7.0 pairs geom_fused with a lower default edge floor because
            geometry-supported scores are intentionally quieter than raw anti.
        edge_fusion_mode: "depth_grad_mul", "depth_var_mul", "grad_var_mul", or
            "blended". Only used when edge_score_mode="geom_fused".
    ...
    Returns dict of (H, W) numpy arrays.  Keys present:
        ...
        edge_score_raw, edge_score_geom, edge_score                    (edge)
        depth_gradient                                                (edge support)
        return_depth_mean, return_depth_near, return_depth_far,
        return_depth_spread, return_mix_score_raw, return_mix_score,
        return_weight_sum, return_weight_expected, return_coverage,
        return_valid_stats, return_expected_spread,
        return_depth_spread_excess,
        beam_mode_gap_ratio, beam_back_ratio                            (BeamReturnLite/BeamEvidenceLite)
    """
    cam = burst.cam
    W, H = cam.width, cam.height
    pixels = burst.pixels
    ...
    if carrier_mode == "x":
        ...
        if carrier_angles is None:
            # v0.7.0: golden-angle four-basis ensemble.  The older cross
            # basis (0/45/90/135°) was useful, but the wave-pattern lab showed
            # that less grid-aligned rotated bases reduce visible stripe locks
            # on broad flat ground while keeping object boundary response.
            golden = _math.pi * (3.0 - _math.sqrt(5.0))  # ~137.5°
            carrier_angles = [i * golden for i in range(4)]
        carrier_coords = [
    ...
    if attenuation_per_meter > 0:
        atten_base = np.where(hit, np.exp(-attenuation_per_meter * t), 0.0)
    else:
        atten_base = hit.astype(np.float64)

    # v0.8.0-alpha5.1 BeamEvidenceLite: every ray may carry a simple beam-shape
    # weight.  Flat weights recover v0.7-style averaging; Gaussian weights make
    # the pixel/beam center contribute more strongly than fringe samples.
    ray_weight = getattr(burst, "ray_weights", None)
    if ray_weight is None:
        ray_weight = np.ones_like(t, dtype=np.float64)
    else:
        ray_weight = np.asarray(ray_weight, dtype=np.float64)
        if ray_weight.shape != t.shape:
            ray_weight = np.ones_like(t, dtype=np.float64)

    atten = atten_base * np.where(hit, ray_weight, 0.0)
    ...
    # ── v0.8.0-alpha5.1 BeamEvidenceLite ────────────────────────────────────
    # Cheap weighted multi-sample return summaries.  This is not full
    # multi-return LiDAR or waveform simulation; it asks whether the weighted
    # rays inside one pixel/beam look like one clean surface or a mixed
    # footprint.
    #
    # alpha5 stabilization:
    #   * use less extreme near/far percentiles (default 15/85 instead of 10/90)
    #   * normalize return strength by expected beam weight -> return_coverage
    #   * gate mixed-return evidence by minimum hit/support
    #   * subtract expected single-surface spread from depth gradient/slope
    #     before turning spread into return_mix_score.
    def _weighted_percentile_by_pixel(values: np.ndarray,
                                      weights: np.ndarray,
                                      percentile: float) -> np.ndarray:
        outp = np.zeros((H, W), dtype=np.float64)
        valid = (in_img & hit & np.isfinite(values) & np.isfinite(weights)
                 & (weights > 0))
        if not np.any(valid):
            return outp
        flat = (py_i[valid] * W + px_i[valid]).astype(np.int64)
        vals = values[valid].astype(np.float64)
        wts = weights[valid].astype(np.float64)
        order = np.lexsort((vals, flat))
        flat_s = flat[order]; vals_s = vals[order]; wts_s = wts[order]
        starts = np.r_[0, np.flatnonzero(np.diff(flat_s)) + 1]
        ends = np.r_[starts[1:], len(flat_s)]
        q = np.clip(float(percentile) / 100.0, 0.0, 1.0)
        out_flat = outp.ravel()
        for s_i, e_i in zip(starts, ends):
            total = float(wts_s[s_i:e_i].sum())
            if total <= 0:
                continue
            target = q * total
            csum = np.cumsum(wts_s[s_i:e_i])
            j = int(np.searchsorted(csum, target, side="left"))
            j = min(max(j, 0), e_i - s_i - 1)
            out_flat[int(flat_s[s_i])] = float(vals_s[s_i + j])
        return outp

    def _beam_split_by_pixel(values: np.ndarray,
                             weights: np.ndarray,
                             expected_spread_map: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """Cheap BeamSplitLite depth-gap test.

        This is not full multi-return LiDAR.  It asks whether the weighted
        sub-rays in one pixel look like one continuous depth group or two
        separated depth groups.  The largest depth gap is compared against the
        expected single-surface spread so sloped/grazing surfaces are less
        likely to become partial occluders.
        """
        split_score = np.zeros((H, W), dtype=np.float64)
        front_depth = np.zeros((H, W), dtype=np.float64)
        back_depth = np.zeros((H, W), dtype=np.float64)
        front_strength = np.zeros((H, W), dtype=np.float64)
        back_strength = np.zeros((H, W), dtype=np.float64)
        mode_gap = np.zeros((H, W), dtype=np.float64)

        valid = (in_img & hit & np.isfinite(values) & np.isfinite(weights)
                 & (weights > 0))
        if not np.any(valid):
            return split_score, front_depth, back_depth, front_strength, back_strength, mode_gap
        ...
        split_f = split_score.ravel()
        front_d_f = front_depth.ravel()
        back_d_f = back_depth.ravel()
        front_s_f = front_strength.ravel()
        back_s_f = back_strength.ravel()
        gap_f = mode_gap.ravel()
        expected_f = expected_spread_map.ravel()

        min_frac = max(float(beam_split_min_weight_frac), 1e-6)
        min_abs_gap = max(float(beam_split_min_abs_gap), 0.0)
        conf_gain = max(float(beam_split_confidence_gain), 1e-6)

        for s_i, e_i in zip(starts, ends):
            n = e_i - s_i
            px_flat = int(flat_s[s_i])
            v = vals_s[s_i:e_i]
            w = wts_s[s_i:e_i]
            total = float(w.sum())
            if total <= 0:
            ...
            # Primary single-surface estimate is still useful when no split is valid.
            front_d_f[px_flat] = float(np.sum(v * w) / max(total, 1e-9))
            front_s_f[px_flat] = 1.0
            ...
            support_frac = np.minimum(left_w, right_w) / max(total, 1e-9)
            support_gate = np.clip(support_frac / min_frac, 0.0, 1.0)

            expected = max(float(expected_f[px_flat]), min_abs_gap, 1e-6)
            # Pick the depth gap that is both large and has non-trivial
            # weighted support on both sides.
            gap_score = (gaps / expected) * support_gate
            best = int(np.argmax(gap_score))
            gap = float(gaps[best])
            lw = float(left_w[best])
            rw = float(right_w[best])
            sf = float(support_frac[best])

            gap_f[px_flat] = gap

            if gap < min_abs_gap or sf < min_frac:
                continue
            ...
            front_d_f[px_flat] = float(np.sum(vf * wf) / max(float(wf.sum()), 1e-9))
            back_d_f[px_flat] = float(np.sum(vb * wb) / max(float(wb.sum()), 1e-9))
            front_s_f[px_flat] = lw / max(total, 1e-9)
            back_s_f[px_flat] = rw / max(total, 1e-9)

            # Confidence rises only after the best gap exceeds expected
            # single-surface spread.  support_gate prevents 1-ray slivers from
            # claiming a hard split.
            excess_ratio = max((gap / expected) - 1.0, 0.0)
            split_f[px_flat] = float(np.clip(excess_ratio * conf_gain * support_gate[best], 0.0, 1.0))

        return split_score, front_depth, back_depth, front_strength, back_strength, mode_gap

    return_weight_sum = sum_light_weight.copy()

    # Expected weight is independent of hit/miss.  It is the beam budget that
    # landed inside each pixel, so return_coverage tells us whether the return
    # summary is well supported instead of exposing the ray layout directly.
    expected_ray_weight = np.where(in_img, ray_weight, 0.0)
    return_weight_expected = agg_real(expected_ray_weight)
    return_coverage = np.where(
        return_weight_expected > 0,
        np.clip(return_weight_sum / np.maximum(return_weight_expected, 1e-9), 0.0, 1.0),
        0.0,
    )

    sum_return_depth = agg_real(np.where(hit, t * light_weight, 0.0))
    return_depth_mean = np.where(return_weight_sum > 0,
                                 sum_return_depth / np.maximum(return_weight_sum, 1e-9),
                                 0.0)
    qn = float(return_q_near)
    qf = float(return_q_far)
    if qf < qn:
        qn, qf = qf, qn
    return_depth_near = _weighted_percentile_by_pixel(t, light_weight, qn)
    return_depth_far = _weighted_percentile_by_pixel(t, light_weight, qf)
    return_depth_spread = np.where(return_weight_sum > 0,
                                   np.maximum(return_depth_far - return_depth_near, 0.0),
                                   0.0)

    return_mix_score_raw = np.where(
        return_weight_sum > 0,
        np.clip(float(return_mix_gain) * return_depth_spread /
                np.maximum(return_depth_mean, 1e-9), 0.0, 1.0),
        0.0,
    )

    # Use local raw depth gradient as a cheap proxy for normal single-surface
    # footprint spread.  A sloped floor or grazing wall naturally creates
    # near/far variation inside a finite beam; only the EXCESS spread should
    # count as mixed-return evidence.
    depth_for_grad = np.where(return_weight_sum > 0, return_depth_mean, 0.0)
    gy_ret, gx_ret = np.gradient(depth_for_grad)
    local_depth_gradient_raw = np.hypot(gx_ret, gy_ret)
    return_expected_spread = (
        float(return_expected_spread_rel) * np.maximum(return_depth_mean, 0.0)
        + float(return_expected_spread_grad) * local_depth_gradient_raw
    )
    return_depth_spread_excess = np.where(
        return_weight_sum > 0,
        np.maximum(return_depth_spread - return_expected_spread, 0.0),
        0.0,
    )

    valid_return_stats = (
        (count >= int(min_return_hit_count))
        & (return_coverage >= float(min_return_coverage))
        & (return_weight_sum > 0)
    )
    return_mix_score = np.where(
        valid_return_stats,
        np.clip(float(return_mix_gain) * return_depth_spread_excess /
                np.maximum(return_depth_mean, 1e-9), 0.0, 1.0),
        0.0,
    )

    # ── v0.8.0-alpha5.1 BeamSplitLite / BeamEvidenceLite ─────────────────
    # A finite beam can have depth spread for two very different reasons:
    #   1) one continuous tilted/grazing surface, or
    #   2) two separated surfaces, like foliage/leg/fence in front of a wall.
    # BeamSplitLite looks for the second case with a cheap weighted largest-
    # gap test.  It complements return_mix_score instead of replacing it.
    (beam_split_score,
     beam_front_depth,
     beam_back_depth,
     beam_front_strength,
     beam_back_strength,
     beam_mode_gap) = _beam_split_by_pixel(t, light_weight, return_expected_spread)

    beam_mode_gap_ratio = np.where(
        return_depth_mean > 1e-9,
        np.clip(beam_mode_gap / np.maximum(return_depth_mean, 1e-9), 0.0, 1.0),
        0.0,
    )
    beam_back_ratio = np.where(
        beam_front_strength > 1e-9,
        np.clip(beam_back_strength / np.maximum(beam_front_strength, 1e-9), 0.0, 1.0),
        0.0,
    )
    beam_mode_confidence = np.where(valid_return_stats, beam_split_score, 0.0)
    # Clean surface confidence: high when the beam has valid support, low
    # corrected mix, and low two-mode evidence.  This is designed to recover
    # solid_surface pixels that alpha1/alpha2 left as uncertain.
    beam_coherence = np.where(
        valid_return_stats,
        return_coverage * np.clip(
            1.0
            - float(beam_coherence_mix_gain) * return_mix_score
            - float(beam_coherence_split_gain) * beam_split_score,
            0.0, 1.0,
        ),
        0.0,
    )
    ...
    out = {
        ...
        # BeamReturnLite weighted return summaries
        "return_depth_mean":   return_depth_mean,
        "return_depth_near":   return_depth_near,
        "return_depth_far":    return_depth_far,
        "return_depth_spread": return_depth_spread,
        "return_mix_score_raw": return_mix_score_raw,
        "return_mix_score":    return_mix_score,
        "return_weight_sum":   return_weight_sum,
        "return_weight_expected": return_weight_expected,
        "return_coverage":     return_coverage,
        "return_valid_stats":  valid_return_stats.astype(np.float64),
        "return_expected_spread": return_expected_spread,
        "return_depth_spread_excess": return_depth_spread_excess,
        # BeamEvidenceLite / BeamSplitLite channels
        "beam_split_score": beam_split_score,
        "beam_front_depth": beam_front_depth,
        "beam_back_depth": beam_back_depth,
        "beam_front_strength": beam_front_strength,
        "beam_back_strength": beam_back_strength,
        "beam_mode_gap": beam_mode_gap,
        "beam_mode_gap_ratio": beam_mode_gap_ratio,
        "beam_back_ratio": beam_back_ratio,
        "beam_mode_confidence": beam_mode_confidence,
        "beam_coherence": beam_coherence,
    }
    # Continuous structure evidence is finalized after all optional channels
    # exist.  v0.7.0 keeps raw anti-wave evidence separate from the geometry-
    # supported edge score used by geom_edge classification.
    ...
    # v0.7.0 edge scoring:
    #   edge_score_raw  = raw anti-wave cross-modal evidence.
    #   edge_score_geom = raw evidence gated by geometric support
    #                     (depth gradient/variance), based on wave-lab v003.
    #   edge_score      = the active score used by geom_edge classification.
    def _norm_support(arr: np.ndarray) -> np.ndarray:
        arr = np.asarray(arr, dtype=np.float64)
        valid = (count > 0) & np.isfinite(arr)
        outn = np.zeros_like(arr, dtype=np.float64)
        if np.any(valid):
            vals = arr[valid]
            lo = np.percentile(vals, 1)
            hi = np.percentile(vals, 99)
            if hi > lo + 1e-12:
                outn[valid] = np.clip((arr[valid] - lo) / (hi - lo), 0, 1)
        return outn

    edge_raw = np.minimum(out["light_anti"], out.get("sound_anti", out["light_anti"]))
    depth_fill = np.where(count > 0, depth_per_pixel, 0.0)
    gy, gx = np.gradient(depth_fill)
    depth_gradient = _norm_support(np.hypot(gx, gy))
    depth_var_support = _norm_support(depth_variance)
    return_mix_support = _norm_support(return_mix_score)
    beam_split_support = _norm_support(out.get("beam_split_score", np.zeros_like(return_mix_score)))

    mode = str(edge_fusion_mode or "depth_grad_mul")
    if mode == "raw":
        edge_geom = edge_raw.copy()
    elif mode == "depth_var_mul":
        edge_geom = edge_raw * depth_var_support
    elif mode == "grad_var_mul":
        edge_geom = edge_raw * np.sqrt(np.clip(depth_gradient * depth_var_support, 0, 1))
    elif mode in ("beam_return_mul", "return_mix_mul"):
        # BeamReturnLite support: a strong mixed-footprint signal can support
        # edges/occlusions even where plain depth-gradient is weak.  It is still
        # only support, not edge truth; anti-wave evidence remains required.
        support = np.maximum(depth_gradient, 0.85 * np.maximum(return_mix_support, beam_split_support))
        edge_geom = edge_raw * np.clip(support, 0, 1)
    elif mode in ("grad_return_mul", "depth_grad_return_mul"):
        support = np.sqrt(np.clip(depth_gradient * np.maximum(return_mix_support, 1e-9), 0, 1))
        edge_geom = edge_raw * support
    elif mode == "blended":
        support = 0.40 * depth_gradient + 0.30 * depth_var_support + 0.30 * return_mix_support
        edge_geom = edge_raw * np.clip(0.35 + 0.65 * support, 0, 1)
    else:  # default / depth_grad_mul
        edge_geom = edge_raw * depth_gradient

    edge_geom = _norm_support(edge_geom)
    out["edge_score_raw"] = np.clip(edge_raw, 0, 1)
    out["depth_gradient"] = depth_gradient
    out["depth_var_support"] = depth_var_support
    out["return_mix_support"] = return_mix_support
    out["beam_split_support"] = beam_split_support
    out["edge_score_geom"] = np.clip(edge_geom, 0, 1)
    if str(edge_score_mode or "geom_fused") in ("raw", "raw_anti", "anti"):
        out["edge_score"] = out["edge_score_raw"].copy()
    else:
        out["edge_score"] = out["edge_score_geom"].copy()
...
ALL_CLASS_LABELS = list(CLASS_COLORS.keys())


CLASSIFICATION_STYLES = {
    "default": CLASS_COLORS,
    # Indoor scenes are mostly solid walls/floors.  Treat solid_surface as
    # background so geom_edge / optical_only / uncertain evidence is readable.
    "indoor_sparse": {
        **CLASS_COLORS,
        "solid_surface": (54, 46, 34),
        "hard_smooth": (62, 66, 70),
        "sky": (8, 14, 22),
    },
    # Material boards should emphasize fine material labels and de-emphasize
    # generic structural fallbacks.
    "material_focus": {
        **CLASS_COLORS,
        "solid_surface": (58, 52, 42),
        "hard_smooth": (80, 90, 96),
        "metal_or_glass": (120, 230, 255),
        "wood_material": (150, 92, 42),
        "stone_hard": (185, 185, 175),
        "soft_material": (255, 185, 105),
        "foliage": (55, 220, 85),
        "sky": (8, 14, 22),
    },
}
...
def _style_classification_image(label_img: Image.Image, style: str = "default") -> Image.Image:
    """Recolor a classification image without changing labels/counts.

    This is purely visualization.  It fixes the washed-out indoor panels
    where solid_surface legitimately dominates the image.
    """
    if not style or style == "default":
        return label_img
    palette = CLASSIFICATION_STYLES.get(style, CLASS_COLORS)
    arr = np.asarray(label_img.convert("RGB")).copy()
    out = arr.copy()
    # Iterate the requested palette so added style keys are honored, but
    # assert that styles only refer to known class labels.
    unknown_style_keys = set(palette.keys()) - set(CLASS_COLORS.keys())
    if unknown_style_keys:
        raise KeyError(f"classification style contains unknown labels: {sorted(unknown_style_keys)}")
    for name, new_color in palette.items():
        old_color = CLASS_COLORS[name]
        mask = np.all(arr == np.array(old_color, dtype=np.uint8), axis=-1)
        out[mask] = np.array(new_color, dtype=np.uint8)
    return Image.fromarray(out)




def edge_score_from_channels(channels: Dict[str, np.ndarray]) -> np.ndarray:
    """Continuous edge evidence used by geom_edge.

    v0.7.0 prefers `channels["edge_score"]`, which may be the geometry-fused
    score.  If an older channel dict is provided, fall back to raw anti-wave
    evidence: min(light_anti, sound_anti).
    """
    if "edge_score" in channels:
        return channels["edge_score"]
    L_a = channels["light_anti"]
    S_a = channels.get("sound_anti")
    return np.minimum(L_a, S_a) if S_a is not None else L_a


def effective_edge_threshold_from_channels(channels: Dict[str, np.ndarray],
                                           edge_anti_min: float = 0.08,
                                           adaptive_edge_percentile: Optional[float] = 90.0,
                                           edge_anti_max: Optional[float] = 0.95
                                           ) -> float:
    """Return the effective geom_edge threshold for these channels/config.

    v0.7.0 adds an upper clamp (`edge_anti_max`, default 0.95).  Without
    this guardrail, tiny/sparse or extremely high-contrast runs could choose an
    adaptive percentile of 1.0, which made edge_confidence and geom_edge_mask
    go black even when edge_score visibly had structure.
    """
    hits = channels.get("hit_count")
    has_hit = (hits > 0) if hits is not None else np.ones_like(channels["light_anti"], dtype=bool)
    edge_anti_min_eff = float(edge_anti_min)
    if adaptive_edge_percentile is not None and np.any(has_hit):
        edge_score = edge_score_from_channels(channels)
        ...
        if np.any(valid_edge):
            try:
                ...
                pass
    if edge_anti_max is not None:
        edge_anti_min_eff = min(edge_anti_min_eff, float(edge_anti_max))
    return float(edge_anti_min_eff)


def edge_confidence_from_channels(channels: Dict[str, np.ndarray],
                                  edge_anti_min: float = 0.08,
                                  adaptive_edge_percentile: Optional[float] = 90.0,
                                  edge_anti_max: Optional[float] = 0.95
                                  ) -> Tuple[np.ndarray, float]:
    """Return continuous edge confidence in [0,1] and the threshold used."""
    score = edge_score_from_channels(channels)
    thr = effective_edge_threshold_from_channels(channels, edge_anti_min, adaptive_edge_percentile, edge_anti_max)
    conf = np.clip((score - thr) / max(1.0 - thr, 1e-9), 0, 1)
    return conf, float(thr)
...
def classify_pixels(channels: Dict[str, np.ndarray],
                    ...
                    partial_occluder_mix_min: float = 0.020,
                    partial_occluder_intensity_max: float = 0.65,
                    foliage_mix_min: float = 0.015,
                    hard_partial_mix_min: float = 0.050,
                    beam_split_min: float = 0.22,
                    beam_split_front_min: float = 0.10,
                    beam_split_back_min: float = 0.07,
                    beam_coherence_solid_min: float = 0.45,
                    partial_occluder_gap_min: float = 0.20,
                    partial_occluder_gap_ratio_min: float = 0.015,
                    partial_occluder_back_ratio_min: float = 0.12,
                    partial_occluder_excess_min: float = 0.05,
                    partial_occluder_coherence_max: float = 0.82,
                    coherent_surface_guard: float = 0.92,
                    classification_priority: str = "sensor",
                    min_return_coverage: float = 0.25,
                    min_return_hit_count: int = 4,
    ...
    Categories are applied in priority order; earlier categories win.

    Base labels:
        sky              : no hit
        partial_occluder : mixed-depth pixel with porous/diluted acoustic signature
        geom_edge        : strong optical + acoustic anti-coherence boundary
        foliage          : low acoustic intensity and foliage-like texture
        hard_smooth      : high ultrasonic/polarization hard-surface cue
        solid_surface    : coherent ordinary surface
        optical_only     : optical edge without acoustic edge
        acoustic_only    : acoustic return with weak optical coherence
        uncertain        : hit pixel not claimed by another rule

    Optional material labels when material_labels=True:
        metal_or_glass, stone_hard, wood_material, soft_material
    ...
    Adaptive edge threshold:
        ...
        threshold becomes max(edge_anti_min, percentile(edge_score,
        adaptive_edge_percentile) over hit pixels).  In v0.7.0 `edge_score` is geometry-fused by default and
        uses a lower floor; percentile thresholding supplies the final scene-
        adaptive cutoff without trusting broad carrier residue.
    """
    ...
    return_mix = channels.get("return_mix_score")
    beam_split = channels.get("beam_split_score")
    beam_front_strength = channels.get("beam_front_strength")
    beam_back_strength = channels.get("beam_back_strength")
    beam_coherence = channels.get("beam_coherence")
    beam_mode_gap = channels.get("beam_mode_gap")
    beam_mode_gap_ratio = channels.get("beam_mode_gap_ratio")
    beam_back_ratio = channels.get("beam_back_ratio")
    return_spread_excess = channels.get("return_depth_spread_excess")
    return_coverage = channels.get("return_coverage")
    return_valid_stats = channels.get("return_valid_stats")
    hits = channels["hit_count"]
    has_hit = hits > 0

    if return_coverage is None:
        return_coverage = np.where(hits > 0, 1.0, 0.0)
    if beam_split is None:
        beam_split = np.zeros_like(L_c)
    if beam_front_strength is None:
        beam_front_strength = np.zeros_like(L_c)
    if beam_back_strength is None:
        beam_back_strength = np.zeros_like(L_c)
    if beam_coherence is None:
        beam_coherence = np.zeros_like(L_c)
    if beam_mode_gap is None:
        beam_mode_gap = np.zeros_like(L_c)
    if beam_mode_gap_ratio is None:
        depth_for_gap = channels.get("return_depth_mean", depth)
        if depth_for_gap is None:
            depth_for_gap = np.zeros_like(L_c)
        beam_mode_gap_ratio = np.where(
            np.asarray(depth_for_gap) > 1e-9,
            np.clip(beam_mode_gap / np.maximum(np.asarray(depth_for_gap), 1e-9), 0.0, 1.0),
            0.0,
        )
    if beam_back_ratio is None:
        beam_back_ratio = np.where(
            beam_front_strength > 1e-9,
            np.clip(beam_back_strength / np.maximum(beam_front_strength, 1e-9), 0.0, 1.0),
            0.0,
        )
    if return_spread_excess is None:
        return_spread_excess = np.zeros_like(L_c)
    if return_valid_stats is None:
        return_valid_stats = (
            (hits >= int(min_return_hit_count))
            & (return_coverage >= float(min_return_coverage))
        ).astype(np.float64)
    valid_return_stats = (
        (return_valid_stats > 0.5)
        & (hits >= int(min_return_hit_count))
        & (return_coverage >= float(min_return_coverage))
    )

    # v0.7.0: use the shared continuous edge score helper so binary geom_edge
    # and edge_score / edge_confidence diagnostics stay consistent.
    edge_anti_min_eff = effective_edge_threshold_from_channels(
        channels, edge_anti_min=edge_anti_min,
        adaptive_edge_percentile=adaptive_edge_percentile,
        edge_anti_max=edge_anti_max
    )
    ...
    edge_metric = edge_score_from_channels(channels)

    if S_c is None:
        # Light-only mode
        solid = has_hit & (L_c > solid_coh_min)
        edge = has_hit & (edge_metric > edge_anti_min_eff)
    ...
    edge = has_hit & (edge_metric > edge_anti_min_eff)
    solid_base = has_hit & (L_c > solid_coh_min) & (S_c > 0.10) & (A_i > solid_intensity_min)

    # BeamCoherence solid recovery: if the weighted beam looks like a clean
    # single surface, do not leave it uncertain merely because one modal channel
    # is weak.  Geom edges still win later because they have priority.
    mix_arr_for_solid = return_mix if return_mix is not None else np.zeros_like(L_c)
    coherent_surface = (
        has_hit
        & valid_return_stats
        & (beam_coherence > float(coherent_surface_guard))
        & (beam_split < max(float(beam_split_min), 0.30))
        & (beam_mode_gap_ratio < max(float(partial_occluder_gap_ratio_min), 0.02))
    )
    solid_recovered = (
        has_hit
        & valid_return_stats
        & (beam_coherence > float(beam_coherence_solid_min))
        & (mix_arr_for_solid < float(hard_partial_mix_min))
        & (beam_split < float(beam_split_min))
        & (S_c > 0.05)
    )
    solid = solid_base | solid_recovered | coherent_surface
    ...
    if A_i is not None:
        var_gate = ((depth_variance > partial_occluder_var_min)
                    if depth_variance is not None else np.zeros_like(has_hit))
        mix_arr = return_mix if return_mix is not None else np.zeros_like(L_c)
        # alpha5: tighten partial_occluder after alpha4 over-fired on
        # coherent ground / horizon-support pixels.  A true partial return must
        # show a split, absolute gap, normalized gap, and non-trivial back/front
        # support while NOT looking like a clean coherent single surface.
        split_gate = (
            valid_return_stats
            & (beam_split > float(beam_split_min))
            & (beam_front_strength > float(beam_split_front_min))
            & (beam_back_strength > float(beam_split_back_min))
            & (beam_back_ratio > float(partial_occluder_back_ratio_min))
            & (beam_mode_gap > float(partial_occluder_gap_min))
            & (beam_mode_gap_ratio > float(partial_occluder_gap_ratio_min))
            & (return_spread_excess > float(partial_occluder_excess_min))
            & (beam_coherence < float(partial_occluder_coherence_max))
            & ~coherent_surface
        )
        foliage_mix_gate = (
            valid_return_stats
            & foliage
            & (mix_arr > float(foliage_mix_min))
            & (beam_back_strength > max(0.03, 0.5 * float(beam_split_back_min)))
            & (beam_back_ratio > 0.5 * float(partial_occluder_back_ratio_min))
            & (beam_coherence < max(float(partial_occluder_coherence_max), 0.88))
            & ~coherent_surface
        )
        generic_mix_gate = (
            valid_return_stats
            & (mix_arr > float(partial_occluder_mix_min))
            & (beam_split > max(0.12, float(beam_split_min) * 0.65))
            & (beam_front_strength > 0.5 * float(beam_split_front_min))
            & (beam_back_strength > 0.75 * float(beam_split_back_min))
            & (beam_back_ratio > 0.75 * float(partial_occluder_back_ratio_min))
            & (beam_mode_gap_ratio > 0.75 * float(partial_occluder_gap_ratio_min))
            & (beam_coherence < min(float(partial_occluder_coherence_max), 0.88))
            & ~coherent_surface
        )
        partial_signal = split_gate | foliage_mix_gate | generic_mix_gate
        # Keep acoustic dilution as support, not a hard veto: clean hard edges
        # are protected mainly by the split/front/back/coherence requirements
        # plus the classification hierarchy below.
        partial_acoustic_ok = (A_i < partial_occluder_intensity_max) | split_gate
    ...
    #   sensor mode     : foliage → partial_occluder → geom_edge → materials → solid
    #   geometry mode   : geom_edge → foliage → partial_occluder → materials → solid
    #
    # alpha5 defaults to sensor interpretation because BeamEvidenceLite is a
    # sensor diagnostic: porous/mixed returns should not be swallowed by generic
    # hard boundary labels.  Geometry extraction users can pass
    # classification_priority="geometry".
    assigned = np.zeros_like(has_hit)
    final = {}
    priority_mode = str(classification_priority or "sensor").lower()
    if priority_mode in ("geometry", "geom", "edge_first"):
        priority_pairs = [
            ("geom_edge",        edge),
            ("foliage",          foliage),
            ("partial_occluder", partial_occluder),
        ]
    else:
        priority_pairs = [
            ("foliage",          foliage),
    ...
    for name, mask in priority_pairs + [
...
def render_intensity(arr: np.ndarray, smooth: bool = True, gamma: float = 0.5,
    ...
    absolute=True renders the [0,1] value directly instead of normalizing by the
    frame max, so a uniform-material surface no longer saturates to full cream.
    """
    src = np.clip(arr, 0.0, 1.0) if absolute else (arr / max(arr.max(), 1e-9))
    a = np.power(np.clip(src, 0, 1), gamma)
...
def render_classification_with_legend(label_img: Image.Image,
    ...
    `style` only changes display colors, not label counts.  Use
    "indoor_sparse" for indoor structure sheets where solid_surface would
    otherwise wash out the panel; use "material_focus" for material boards.
    """
    label_img = _style_classification_image(label_img, style=style)
    legend_colors = CLASSIFICATION_STYLES.get(style, CLASS_COLORS)
    ...
    for r, row in enumerate(rows):
        ...
        for name, text, text_w in row:
            color = legend_colors.get(name, CLASS_COLORS.get(name, (200, 200, 200)))
...
def fire_burst_wave(cam: Camera, prims: List[Primitive], n_samples: int,
                    ...
                    min_return_coverage: float = 0.25,
                    min_return_hit_count: int = 4,
                    impedance_fn=material_prior_acoustic,
                    impedance_ult_fn=material_prior_ultrasonic,
                    polarization_fn=material_prior_polarization
    ...
    for k in range(n_stacked_bursts):
        b = fire_burst(
            cam, prims, n_samples, seed=seed + k,
            beam_profile=beam_profile,
            beam_width=beam_width,
            min_ray_weight=min_ray_weight,
        )
        if sensor_noise:
            b = add_sensor_noise(
        ...
        ch = compute_wave_channels(
            ...
            edge_score_mode=edge_score_mode,
            ...
            return_mix_gain=return_mix_gain,
            return_q_near=return_q_near,
            return_q_far=return_q_far,
            return_expected_spread_rel=return_expected_spread_rel,
            return_expected_spread_grad=return_expected_spread_grad,
            min_return_coverage=min_return_coverage,
            min_return_hit_count=min_return_hit_count,
...
def _build_material_target_board_scene() -> List[Primitive]:
    ...
    v0.7.0: The earlier notebook material_targets scene could under-expose
...
SENSOR_PRESETS: Dict[str, Dict] = {
    ...
    "indoor_structure": {
        ...
        "classification_style": "indoor_sparse",
        "panels": [
            "shaded", "depth", "light_anti", "sound_anti",
            "edge_score_raw", "edge_score_geom",
            "geom_edge_overlay", "classification",
        ],
    },
    "outdoor_occlusion": {
    ...
    "material_scan": {
        ...
        "classification_style": "material_focus",
        "panels": [
            "shaded", "depth", "geom_edge_overlay", "geom_edge_mask",
            "acoustic_intensity", "ultrasonic_intensity", "acoustic_texture", "polarization",
            "classification", "material_core", "material_filled",
        ],
        "edge_fill_radius": 1,
        "edge_fill_min_fraction": 0.65,
        "edge_fill_fine_priority": False,
    },
    "compact_diagnostic": {
        "description": "Readable compact diagnostic sheet for normal scene inspection.",
        "camera": "corner_eyelevel",
        "auto_frame": True,
        "min_coverage": 0.35,
        "min_depth_span": 3.0,
        ...
        "panels": [
            "shaded", "depth", "light_anti", "sound_anti",
            "edge_score_raw", "edge_score_geom", "edge_confidence", "geom_edge_overlay", "classification",
        ],
    },
    "edge_debug": {
        "description": "Focused edge/boundary debug sheet with score, confidence, mask, and overlay.",
        "camera": "corner_eyelevel",
        "auto_frame": True,
        "min_coverage": 0.35,
        "min_depth_span": 3.0,
        ...
        "classification_style": "indoor_sparse",
        "panels": [
            "shaded", "edge_score_raw", "edge_score_geom", "edge_confidence",
            "geom_edge_overlay", "depth_variance", "classification", "material_core",
        ],
    },
    "full_diagnostic": {
        ...
        "panels": [
            ...
            "edge_score_raw", "edge_score_geom", "polarization", "classification",
        ],
    },
    "beam_return_debug": {
        "description": "v0.8-alpha5.1 BeamEvidenceLite guard patch: coherent-surface guard + gap/back-ratio partial gating.",
        "camera": "corner_eyelevel",
        "auto_frame": True,
        "min_coverage": 0.35,
        "min_depth_span": 3.0,
        ...
        "return_q_near": 15.0,
        "return_q_far": 85.0,
        "return_expected_spread_rel": 0.01,
        "return_expected_spread_grad": 0.10,
        "min_return_coverage": 0.25,
        "min_return_hit_count": 4,
        "beam_split_min_weight_frac": 0.10,
        "beam_split_min_abs_gap": 0.10,
        "beam_split_confidence_gain": 0.50,
        "beam_coherence_mix_gain": 6.0,
        "beam_coherence_split_gain": 0.80,
        "beam_split_min": 0.38,
        "beam_split_front_min": 0.10,
        "beam_split_back_min": 0.08,
        "beam_coherence_solid_min": 0.45,
        "partial_occluder_gap_min": 0.20,
        "partial_occluder_gap_ratio_min": 0.015,
        "partial_occluder_back_ratio_min": 0.12,
        "partial_occluder_excess_min": 0.05,
        "partial_occluder_coherence_max": 0.82,
        "coherent_surface_guard": 0.92,
        "classification_priority": "sensor",
        "carrier_mode": "ensemble",
        "carrier_strength": 0.50,
        "edge_score_mode": "geom_fused",
        "edge_fusion_mode": "beam_return_mul",
        "depth_wavelength": 1.0,
        "edge_anti_min": 0.08,
        "adaptive_edge_percentile": 92.0,
        "edge_anti_max": 0.95,
        "partial_occluder_mix_min": 0.020,
        "foliage_mix_min": 0.015,
        "hard_partial_mix_min": 0.050,
        "include_ultrasonic": True,
        "include_polarization": True,
        "attenuation_per_meter": 0.0,
        "material_labels": False,
        "classification_style": "indoor_sparse",
        "panels": [
            "shaded", "depth", "return_depth_mean", "return_depth_spread_excess",
            "return_mix_score", "beam_split_score", "beam_front_strength",
            "beam_back_strength", "beam_back_ratio", "beam_mode_gap",
            "beam_mode_gap_ratio", "beam_coherence", "edge_score_geom", "classification",
        ],
    },
}
...
def make_camera_for_preset(prims: List[Primitive],
    ...
    return Camera(
        ...
        # Forward lens-specific params so manual fisheye / non-topdown ortho
        # captures honour the requested coverage instead of silently using the
        # Camera defaults (180 deg / size 8).
        fisheye_fov_deg=float(cfg.get("fisheye_fov_deg", 180.0)),
        ortho_size=float(cfg.get("ortho_size", 8.0)),
    )
...
CLASSIFIER_KWARG_NAMES = [
    ...
    "partial_occluder_mix_min",
    "partial_occluder_intensity_max",
    "foliage_mix_min",
    "hard_partial_mix_min",
    "beam_split_min",
    "beam_split_front_min",
    "beam_split_back_min",
    "beam_coherence_solid_min",
    "partial_occluder_gap_min",
    "partial_occluder_gap_ratio_min",
    "partial_occluder_back_ratio_min",
    "partial_occluder_excess_min",
    "partial_occluder_coherence_max",
...
def classifier_kwargs_from_config(cfg: Dict) -> Dict:
    ...
    out.setdefault("edge_anti_min", float(cfg.get("edge_anti_min", 0.08)))
    out.setdefault("adaptive_edge_percentile", cfg.get("adaptive_edge_percentile", 90.0))
    _edge_max_default = cfg.get("edge_anti_max", 0.95)
    if _edge_max_default is not None:
        out.setdefault("edge_anti_max", float(_edge_max_default))
...
def classification_label_array(label_img: Image.Image) -> np.ndarray:
    """Return an object array of label names from a classification RGB image."""
    arr = np.asarray(label_img.convert("RGB"))
    H, W = arr.shape[:2]
    labels = np.full((H, W), "unknown", dtype=object)
...
# ─────────────────────────────────────────────────────────────────────────
#  GEOM_EDGE AS FIRST-CLASS STRUCTURE / BOUNDARY LAYER (v0.7.0)
# ─────────────────────────────────────────────────────────────────────────
def label_array_to_image(label_arr: np.ndarray,
                         style: str = "default",
                         unknown_color: Tuple[int, int, int] = (8, 16, 26)
                         ) -> Image.Image:
    """Render a 2D array of label strings using a classification style palette."""
    palette = CLASSIFICATION_STYLES.get(style, CLASS_COLORS)
    H, W = label_arr.shape
    out = np.zeros((H, W, 3), dtype=np.uint8)
    out[..., :] = np.array(unknown_color, dtype=np.uint8)
    for name, color in palette.items():
        out[label_arr == name] = np.array(color, dtype=np.uint8)
    return Image.fromarray(out)


def classify_to_label_array(channels: Dict[str, np.ndarray],
                            cfg_or_kwargs: Optional[Dict] = None
                            ) -> Tuple[np.ndarray, Dict[str, int], Image.Image]:
    """Classify channels and return (label array, counts, raw color image)."""
    kwargs = classifier_kwargs_from_config(cfg_or_kwargs or {})
    label_img, counts = classify_pixels(channels, **kwargs)
    return classification_label_array(label_img), counts, label_img


def geom_edge_mask_from_labels(label_arr: np.ndarray) -> np.ndarray:
    """Boolean mask of geom_edge pixels."""
    return label_arr == "geom_edge"


def render_geom_edge_mask(label_arr: np.ndarray) -> Image.Image:
    """Render geom_edge as a white mask on black."""
    mask = geom_edge_mask_from_labels(label_arr)
    out = np.zeros((*mask.shape, 3), dtype=np.uint8)
    out[mask] = np.array([245, 245, 245], dtype=np.uint8)
    return Image.fromarray(out)


def render_geom_edge_overlay(label_arr: np.ndarray,
                             base_img: Image.Image,
                             edge_color: Tuple[int, int, int] = (255, 60, 45),
                             alpha: float = 0.80) -> Image.Image:
    """Overlay geom_edge pixels on top of a shaded/material image."""
    base = np.asarray(base_img.convert("RGB")).astype(np.float64)
    edge = geom_edge_mask_from_labels(label_arr)
    color = np.array(edge_color, dtype=np.float64)
    base[edge] = base[edge] * (1.0 - alpha) + color * alpha
    return Image.fromarray(np.clip(base, 0, 255).astype(np.uint8))


def material_core_label_array(label_arr: np.ndarray,
                              edge_label: str = "geom_edge",
                              blank_label: str = "sky") -> np.ndarray:
    """Return a copy where geom_edge pixels are blanked for material-core scoring."""
    out = label_arr.copy()
    out[out == edge_label] = blank_label
    return out


def fill_geom_edges_from_neighbors(label_arr: np.ndarray,
                                   radius: int = 1,
                                   min_fraction: float = 0.65,
                                   fine_priority: bool = False,
                                   keep_if_tie: bool = True
                                   ) -> Tuple[np.ndarray, Dict[str, float]]:
    """Fill geom_edge pixels from confident neighboring labels.

    This is a derived material/visualization map only.  The original edge
    mask should be preserved as the structure layer.  Edges are filled only
    when nearby non-edge labels agree strongly enough.
    """
    radius = max(1, int(radius))
    min_fraction = float(min_fraction)
    H, W = label_arr.shape
    out = label_arr.copy()
    edge = label_arr == "geom_edge"
    ys, xs = np.where(edge)
    ignore = {"sky", "unknown", "uncertain", "geom_edge"}
    fine_labels = ["metal_or_glass", "wood_material", "stone_hard",
                   "soft_material", "foliage"]
    filled = 0
    kept = 0
    ...
    for y, x in zip(ys, xs):
        ...
        neigh = label_arr[y0:y1, x0:x1].ravel()
        vals = [str(v) for v in neigh if str(v) not in ignore]
        if not vals:
            kept += 1
            continue
        if fine_priority:
            fine_vals = [v for v in vals if v in fine_labels]
            if fine_vals:
                vals = fine_vals
        uniq, cnt = np.unique(np.asarray(vals, dtype=object), return_counts=True)
        order = np.argsort(cnt)[::-1]
        top = str(uniq[order[0]])
        frac = float(cnt[order[0]] / max(len(vals), 1))
        tied = len(order) > 1 and cnt[order[0]] == cnt[order[1]]
        if frac >= min_fraction and not (keep_if_tie and tied):
            out[y, x] = top
            filled += 1
            fill_counts[top] = fill_counts.get(top, 0) + 1
        else:
            kept += 1
    stats = {
        "edge_pixels": int(len(xs)),
        "filled_pixels": int(filled),
        "kept_edge_pixels": int(kept),
        "fill_rate": float(filled / max(len(xs), 1)),
    }
    for k, v in sorted(fill_counts.items()):
        stats[f"filled_as_{k}"] = int(v)
    return out, stats


def _material_label_agreement_from_label_arr(label_arr: np.ndarray,
                                             burst: Burst,
                                             prims: List[Primitive],
                                             expected_labels: Optional[Dict[str, str]] = None,
                                             min_pixels: int = 5,
                                             suffix: str = "") -> List[Dict]:
    """Shared material agreement report from a precomputed label array."""
    expected_labels = dict(expected_labels or DEFAULT_MATERIAL_EXPECTED_LABELS)
    masks = dominant_piece_type_masks(burst, prims)
    rows = []
    for piece_type, mask in sorted(masks.items()):
        n = int(mask.sum())
        if n < int(min_pixels):
            continue
        labels_here = label_arr[mask]
        # Treat blanked sky pixels as ignored for core-style agreement.
        valid_here = labels_here[labels_here != "sky"]
        valid_n = int(len(valid_here))
        score_labels = valid_here if valid_n > 0 else labels_here
        denom = max(len(score_labels), 1)
        uniq, cnt = np.unique(score_labels, return_counts=True)
        order = np.argsort(cnt)[::-1]
        top_label = str(uniq[order[0]]) if len(order) else "none"
        top_frac = float(cnt[order[0]] / denom) if len(order) else 0.0
        expected = expected_labels.get(str(piece_type), None)
        expected_frac = None
        if expected is not None:
            expected_frac = float(np.sum(score_labels == expected) / denom)
        row = {
            "piece_type": str(piece_type),
            "pixel_count": n,
            "valid_pixel_count": valid_n,
            "ignored_pixel_count": int(n - valid_n),
            "expected_label": expected,
            "expected_label_frac": expected_frac,
            "top_label": top_label,
            "top_label_frac": top_frac,
            "report_suffix": suffix,
        }
        for lab in ALL_CLASS_LABELS:
            row[lab + "_frac"] = float(np.sum(score_labels == lab) / denom)
        rows.append(row)
    return rows


def material_core_agreement_report(burst: Burst,
                                   channels: Dict[str, np.ndarray],
                                   prims: List[Primitive],
                                   classify_kwargs: Optional[Dict] = None,
                                   expected_labels: Optional[Dict[str, str]] = None,
                                   min_pixels: int = 5,
                                   label_arr: Optional[np.ndarray] = None) -> List[Dict]:
    """Material agreement after ignoring geom_edge pixels.

    This mirrors common segmentation practice: boundaries are evaluated
    separately from region/core material identity.
    """
    if label_arr is None:
        label_arr, _, _ = classify_to_label_array(channels, classify_kwargs or {})
    core = material_core_label_array(label_arr)
    return _material_label_agreement_from_label_arr(
        core, burst, prims, expected_labels=expected_labels,
        min_pixels=min_pixels, suffix="core_ignore_geom_edge"
    )


def material_filled_agreement_report(burst: Burst,
                                     channels: Dict[str, np.ndarray],
                                     prims: List[Primitive],
                                     classify_kwargs: Optional[Dict] = None,
                                     expected_labels: Optional[Dict[str, str]] = None,
                                     ...
                                     fine_priority: bool = False,
                                     label_arr: Optional[np.ndarray] = None) -> List[Dict]:
    """Material agreement after conservative neighbor-fill of geom_edge pixels."""
    if label_arr is None:
        label_arr, _, _ = classify_to_label_array(channels, classify_kwargs or {})
    filled, stats = fill_geom_edges_from_neighbors(
        label_arr, radius=radius, min_fraction=min_fraction,
        fine_priority=fine_priority
    )
    rows = _material_label_agreement_from_label_arr(
        filled, burst, prims, expected_labels=expected_labels,
        min_pixels=min_pixels, suffix="filled_geom_edge"
    )
    for row in rows:
        row.update({f"fill_{k}": v for k, v in stats.items()})
    return rows


def boundary_adjacency_report(channels: Dict[str, np.ndarray],
                              classify_kwargs: Optional[Dict] = None,
                              radius: int = 1,
                              min_count: int = 1,
                              label_arr: Optional[np.ndarray] = None) -> List[Dict]:
    """Report which labels appear on either side of geom_edge pixels.

    If an edge pixel sees one dominant non-edge label around it, the pair is
    label↔label.  If it sees multiple labels, every adjacent pair is counted;
    total adjacency counts can therefore exceed the number of geom_edge pixels.
    """
    if label_arr is None:
        label_arr, _, _ = classify_to_label_array(channels, classify_kwargs or {})
    H, W = label_arr.shape
    ...
    for y, x in zip(ys, xs):
        y0, y1 = max(0, y - radius), min(H, y + radius + 1)
        x0, x1 = max(0, x - radius), min(W, x + radius + 1)
        vals = sorted(set(str(v) for v in label_arr[y0:y1, x0:x1].ravel()
                          if str(v) not in ignore))
        if len(vals) == 1:
            pair = (vals[0], vals[0])
    ...
    for (a, b), c in sorted(counts.items(), key=lambda kv: kv[1], reverse=True):
        if c < min_count:
            continue
        rows.append({
            "label_a": a,
            "label_b": b,
            "edge_pixel_adjacency_count": int(c),
            "fraction_of_adjacencies": float(c / max(total, 1)),
        })
    return rows


def _connected_components_bool(mask: np.ndarray, min_size: int = 1) -> Tuple[int, int, int]:
    """Tiny 8-connected component counter for boolean masks.

    Returns (n_components, n_components_ge_min_size, largest_size).
    """
    H, W = mask.shape
    ...
    for y in range(H):
        for x in range(W):
            ...
            if size >= int(min_size):
                big += 1
    return int(n), int(big), int(largest)


def structure_density_report(channels: Dict[str, np.ndarray],
                             classify_kwargs: Optional[Dict] = None,
                             min_component_size: int = 20,
                             label_arr: Optional[np.ndarray] = None,
                             counts: Optional[Dict[str, int]] = None) -> Dict:
    """Scene-level geom_edge/structure health metrics."""
    if label_arr is None or counts is None:
        label_arr, counts, _ = classify_to_label_array(channels, classify_kwargs or {})
    hit_px = sum(int(v) for k, v in counts.items() if k != "sky")
    edge = label_arr == "geom_edge"
    n_comp, n_big, largest = _connected_components_bool(edge, min_size=min_component_size)
    out = {
        "hit_labeled_pixels": int(hit_px),
        "geom_edge_pixels": int(edge.sum()),
        "geom_edge_frac": float(edge.sum() / max(hit_px, 1)),
        "edge_components": int(n_comp),
        "edge_components_ge_min_size": int(n_big),
        "largest_edge_component_px": int(largest),
    }
    for name in ["partial_occluder", "optical_only", "acoustic_only",
                 "solid_surface", "hard_smooth", "uncertain"]:
        out[name + "_frac"] = float(counts.get(name, 0) / max(hit_px, 1))
    return out



DEFAULT_MATERIAL_EXPECTED_LABELS = {
    "metal": "metal_or_glass",
    "glass": "metal_or_glass",
    "wood": "wood_material",
    "painted_wood": "wood_material",
    "wood_floor": "wood_material",
    "wood_wall": "wood_material",
    "bar_counter": "wood_material",
    "shelf": "wood_material",
    "chair": "wood_material",
    "table_top": "wood_material",
    "table_leg": "wood_material",
    "stone": "stone_hard",
    "concrete": "stone_hard",
    "brick": "stone_hard",
    "cloth": "soft_material",
    "fabric": "soft_material",
    "carpet": "soft_material",
    "curtain": "soft_material",
    "foliage": "foliage",
    "canopy": "foliage",
}


def material_label_agreement_report(burst: Burst,
                                    channels: Dict[str, np.ndarray],
                                    prims: List[Primitive],
                                    classify_kwargs: Optional[Dict] = None,
                                    expected_labels: Optional[Dict[str, str]] = None,
                                    min_pixels: int = 5,
                                    label_arr: Optional[np.ndarray] = None) -> List[Dict]:
    """Report raw material-label agreement, including geom_edge pixels.

    This is the "raw" companion to material_core_agreement_report and
    material_filled_agreement_report.
    """
    if label_arr is None:
        label_arr, _, _ = classify_to_label_array(channels, classify_kwargs or {})
    return _material_label_agreement_from_label_arr(
        label_arr, burst, prims,
        expected_labels=expected_labels, min_pixels=min_pixels, suffix="raw"
    )
...
def _render_panel(panel_name: str,
                  ...
                  prims: List[Primitive],
                  cfg: Dict,
                  classified: Optional[Tuple[np.ndarray, Dict[str, int], Image.Image]] = None) -> Tuple[Image.Image, str]:
    ...
    if classified is None:
        classified = classify_to_label_array(channels, cfg)
    label_arr, counts, raw_label_img = classified
    if name == "classification":
        return render_classification_with_legend(raw_label_img, counts, style=cfg.get("classification_style", "default")), "classification"
    if name == "geom_edge_mask":
        return render_geom_edge_mask(label_arr), "geom_edge mask"
    if name == "geom_edge_overlay":
        base = render_burst(burst, mode="shaded", bg="#08101a")
        return render_geom_edge_overlay(label_arr, base), "geom_edge overlay"
    if name == "material_core":
        core = material_core_label_array(label_arr)
        return label_array_to_image(core, style=cfg.get("classification_style", "default")), "material core"
    if name == "material_filled":
        filled, _ = fill_geom_edges_from_neighbors(
            label_arr,
            radius=int(cfg.get("edge_fill_radius", 1)),
            min_fraction=float(cfg.get("edge_fill_min_fraction", 0.65)),
            fine_priority=bool(cfg.get("edge_fill_fine_priority", False)),
        )
        return label_array_to_image(filled, style=cfg.get("classification_style", "default")), "material filled"
    if name in ("acoustic_intensity", "ultrasonic_intensity"):
        return render_intensity(channels.get(name, np.zeros_like(channels["hit_count"])),
                                absolute=True), name
    if name in channels:
        arr = channels[name]
        if name in ("depth_per_pixel", "depth_variance",
                    "return_depth_mean", "return_depth_near",
                    "return_depth_far", "return_depth_spread",
                    "return_weight_sum", "return_weight_expected",
                    "return_expected_spread", "return_depth_spread_excess",
                    "beam_front_depth", "beam_back_depth", "beam_mode_gap"):
            arr = _normalize_channel_for_render(arr)
        # Material channels are already in [0,1] and their magnitude is
        # meaningful — render absolutely so a uniform field doesn't saturate.
        absolute = name in ("acoustic_softness", "acoustic_texture", "polarization")
        return render_channel(arr, absolute=absolute), name
...
# ──────────────────────────────────────────────────────────────────────────
# Within-frame scout-then-fill adaptive sampler (opt-in via sampler="scout_fill").
#
# Spends ~10% of the ray budget scouting uniformly to find where rays return,
# then concentrates the rest around those confirmed hits. Gives noticeably
# better object/edge resolution per ray when the subject is small/distant in
# frame; on scenes that already fill the view it ~matches uniform. It is NOT
# faster (same ray count, and object rays cost more to cast than sky rays) —
# the benefit is quality-per-budget, not speed. Anchored to confirmed returns,
# so it never wastes budget exploring empty sky.
# ──────────────────────────────────────────────────────────────────────────
def _box_blur2d(a, r):
    ...
    def _b1(x):
        H, W = x.shape
        cs = np.concatenate([np.zeros((H, 1)), np.cumsum(x, axis=1)], axis=1)
        out = np.empty_like(x)
        for j in range(W):
            lo = max(0, j - r); hi = min(W, j + r + 1)
            out[:, j] = (cs[:, hi] - cs[:, lo]) / (hi - lo)
        return out
    return _b1(_b1(a).T).T


def _scout_priority(scout_burst, W, H, blur_px=6):
    hit = scout_burst.depths < INF
    px = scout_burst.pixels[hit, 0].astype(int); py = scout_burst.pixels[hit, 1].astype(int)
    ok = (px >= 0) & (px < W) & (py >= 0) & (py < H)
    g = np.zeros((H, W), float); g[py[ok], px[ok]] = 1.0
    g = _box_blur2d(g, int(blur_px))
    m = g.max()
    return g / m if m > 1e-9 else g


def _scout_fill_pass(cam, prims, n_fill, seed, priority, beam_profile="gaussian",
                     beam_width=0.35, min_ray_weight=0.05, candidate_mult=3.0,
                     priority_gain=2.5, base_score=0.02, uniform_fraction=0.1):
    rng = np.random.default_rng(seed)
    gen = LENS_REGISTRY.get(cam.lens, rays_pinhole)
    cand = max(n_fill, int(n_fill * candidate_mult))
    O, D, P = gen(cam, cand, rng)
    px = np.clip(P[:, 0].astype(int), 0, cam.width - 1)
    py = np.clip(P[:, 1].astype(int), 0, cam.height - 1)
    score = base_score + priority_gain * priority[py, px]
    keep_n = min(n_fill, len(O)); idx = np.arange(len(O))
    uni_n = int(np.clip(uniform_fraction, 0, 0.95) * keep_n)
    uni = rng.choice(idx, uni_n, replace=False) if uni_n else np.zeros(0, dtype=np.int64)
    mask = np.ones(len(O), bool); mask[uni] = False; rem = idx[mask]
    w_n = keep_n - len(uni)
    if w_n > 0:
        rs = np.clip(score[rem], 0, None); s = rs.sum()
        wk = (rng.choice(rem, w_n, replace=False, p=rs / s) if s > 1e-9
              else rng.choice(rem, w_n, replace=False))
        keep = np.concatenate([uni, wk])
    else:
        keep = uni
    O, D, P = O[keep], D[keep], P[keep]
    rw = beam_ray_weights(P, beam_profile=beam_profile, beam_width=beam_width, min_weight=min_ray_weight)
    t, color, normal, pid = cast_rays(O, D, prims, rng=rng)
    hit = t < INF
    return Burst(cam=cam, pixels=P, origins=O, dirs=D, depths=t, colors=color,
                 normals=normal, piece_ids=pid, ray_weights=rw,
                 coverage=float(hit.mean()) if len(t) else 0.0,
                 unique_pieces=int(len(np.unique(pid[hit]))) if hit.any() else 0)


def _combine_bursts(b1, b2):
    c = lambda a, b: np.concatenate([a, b], 0)
    depths = c(b1.depths, b2.depths); hit = depths < INF; pid = c(b1.piece_ids, b2.piece_ids)
    return Burst(cam=b1.cam, pixels=c(b1.pixels, b2.pixels), origins=c(b1.origins, b2.origins),
                 dirs=c(b1.dirs, b2.dirs), depths=depths, colors=c(b1.colors, b2.colors),
                 normals=c(b1.normals, b2.normals), piece_ids=pid,
                 ray_weights=c(b1.ray_weights, b2.ray_weights),
                 coverage=float(hit.mean()) if len(depths) else 0.0,
                 unique_pieces=int(len(np.unique(pid[hit]))) if hit.any() else 0)


def scout_then_fill(cam, prims, n_samples, seed, scout_frac=0.1,
                    beam_profile="gaussian", beam_width=0.35, min_ray_weight=0.05):
    """Within-frame scout-then-fill burst (see note above)."""
    n_scout = max(1, int(n_samples * scout_frac))
    n_fill = max(0, n_samples - n_scout)
    scout = fire_burst(cam, prims, n_samples=n_scout, seed=seed,
                       beam_profile=beam_profile, beam_width=beam_width, min_ray_weight=min_ray_weight)
    if n_fill == 0:
        return scout
    pri = _scout_priority(scout, cam.width, cam.height)
    fill = _scout_fill_pass(cam, prims, n_fill, seed + 1, pri,
                            beam_profile=beam_profile, beam_width=beam_width, min_ray_weight=min_ray_weight)
    return _combine_bursts(scout, fill)


def _preset_burst(cam, prims, n_samples, seed, cfg, sampler="uniform"):
    bp = cfg.get("beam_profile", "gaussian"); bw = float(cfg.get("beam_width", 0.35))
    mw = float(cfg.get("min_ray_weight", 0.05))
    if sampler == "scout_fill":
        return scout_then_fill(cam, prims, n_samples, seed, beam_profile=bp, beam_width=bw, min_ray_weight=mw)
    return fire_burst(cam, prims, n_samples=n_samples, seed=seed, beam_profile=bp, beam_width=bw, min_ray_weight=mw)
...
def run_sensor_preset(prims: List[Primitive],
    ...
    cfg = get_sensor_preset(preset_name, **preset_overrides)
    warnings = []
    if bool(cfg.get("material_labels", False)):
        # Fine material labels depend on ultrasonic and polarization evidence.
        if not bool(cfg.get("include_ultrasonic", False)):
            warnings.append("material_labels=True forces include_ultrasonic=True")
        if not bool(cfg.get("include_polarization", False)):
            warnings.append("material_labels=True forces include_polarization=True")
        cfg["include_ultrasonic"] = True
        cfg["include_polarization"] = True
    cam = make_camera_for_preset(prims, preset_name, **preset_overrides)
    frame_diagnostics = {"enabled": False}
    autoframe_runtime_seconds = 0.0
    if bool(cfg.get("auto_frame", False)) and bool(cfg.get("wave", True)):
        ...
        autoframe_runtime_seconds = _time.time() - _af_t0
    n_samples = int(cam.width * cam.height * cfg.get("rays_per_pixel", 1))
    render_t0 = _time.time()
    ...
    if not bool(cfg.get("wave", True)):
        burst = _preset_burst(cam, prims, n_samples, seed, cfg, sampler)
        last_burst = burst
        imgs, labels = [], []
        ...
        pilot = _preset_burst(cam, prims, n_samples, seed, cfg, sampler)
        lref_value = estimate_L_ref_from_burst(pilot) if L_ref == "auto" else float(L_ref)
        for k in range(stack):
            b = pilot if k == 0 else _preset_burst(cam, prims, n_samples, seed + k, cfg, sampler)
            ch = compute_wave_channels(
                b, prims=prims,
                ...
                carrier_angles=cfg.get("carrier_angles", None),
                edge_score_mode=cfg.get("edge_score_mode", "geom_fused"),
                edge_fusion_mode=cfg.get("edge_fusion_mode", "depth_grad_mul"),
                include_sound=True,
                ...
                return_mix_gain=float(cfg.get("return_mix_gain", 1.0)),
                return_q_near=float(cfg.get("return_q_near", 15.0)),
                return_q_far=float(cfg.get("return_q_far", 85.0)),
                return_expected_spread_rel=float(cfg.get("return_expected_spread_rel", 0.01)),
                return_expected_spread_grad=float(cfg.get("return_expected_spread_grad", 0.10)),
                min_return_coverage=float(cfg.get("min_return_coverage", 0.25)),
                min_return_hit_count=int(cfg.get("min_return_hit_count", 4)),
                beam_split_min_weight_frac=float(cfg.get("beam_split_min_weight_frac", 0.10)),
                beam_split_min_abs_gap=float(cfg.get("beam_split_min_abs_gap", 0.10)),
                beam_split_confidence_gain=float(cfg.get("beam_split_confidence_gain", 0.50)),
                beam_coherence_mix_gain=float(cfg.get("beam_coherence_mix_gain", 6.0)),
                beam_coherence_split_gain=float(cfg.get("beam_coherence_split_gain", 0.80)),
                impedance_fn=impedance_fn,
                impedance_ult_fn=impedance_ult_fn,
                polarization_fn=polarization_fn,
            )
        ...
        # Add threshold-dependent edge confidence as a derived channel.
        edge_conf, edge_thr = edge_confidence_from_channels(
            channels,
            edge_anti_min=float(cfg.get("edge_anti_min", 0.20)),
            adaptive_edge_percentile=cfg.get("adaptive_edge_percentile", 90.0),
            edge_anti_max=cfg.get("edge_anti_max", 0.95),
        )
        channels["edge_confidence"] = edge_conf
        classified = classify_to_label_array(channels, cfg)

        imgs, labels = [], []
        for panel in cfg.get("panels", ["shaded", "depth", "light_anti", "sound_anti", "classification"]):
            im, lab = _render_panel(panel, last_burst, channels, prims, cfg, classified=classified)
            imgs.append(im); labels.append(lab)
        cols = 4 if len(imgs) >= 8 else min(3, max(1, len(imgs)))
        sheet = composite_grid(imgs, labels, cols=cols)

    render_runtime_seconds = _time.time() - render_t0

    stats = burst_depth_stats(last_burst)
    counts = None
    if classified is not None:
        _, counts, _ = classified

    report_t0 = _time.time()
    _classifier_kwargs = classifier_kwargs_from_config(cfg) if channels is not None else {}
    _label_arr = classified[0] if classified is not None else None
    _material_report = material_channel_report(last_burst, channels, prims) if channels is not None else []
    _material_label_agreement = material_label_agreement_report(
        last_burst, channels, prims, _classifier_kwargs, label_arr=_label_arr
    ) if channels is not None else []
    _material_core_agreement = material_core_agreement_report(
        last_burst, channels, prims, _classifier_kwargs, label_arr=_label_arr
    ) if channels is not None else []
    _material_filled_agreement = material_filled_agreement_report(
        last_burst, channels, prims, _classifier_kwargs,
        radius=int(cfg.get("edge_fill_radius", 1)),
        min_fraction=float(cfg.get("edge_fill_min_fraction", 0.65)),
        fine_priority=bool(cfg.get("edge_fill_fine_priority", False)),
        label_arr=_label_arr
    ) if channels is not None else []
    _boundary_adjacency = boundary_adjacency_report(
        channels, _classifier_kwargs, label_arr=_label_arr
    ) if channels is not None else []
    _structure_density = structure_density_report(
        channels, _classifier_kwargs, label_arr=_label_arr,
        counts=classified[1] if classified is not None else None
    ) if channels is not None else {}
    report_runtime_seconds = _time.time() - report_t0
    ...
    diagnostics = {
        ...
        "L_ref": float(lref_value) if lref_value is not None else None,
        "edge_threshold_used": float(edge_thr) if channels is not None else None,
        "edge_score_p90": (float(np.percentile(channels["edge_score"][channels["hit_count"] > 0], 90)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "edge_score_p95": (float(np.percentile(channels["edge_score"][channels["hit_count"] > 0], 95)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "edge_score_p99": (float(np.percentile(channels["edge_score"][channels["hit_count"] > 0], 99)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "edge_score_raw_p95": (float(np.percentile(channels.get("edge_score_raw", channels["edge_score"])[channels["hit_count"] > 0], 95)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "edge_score_geom_p95": (float(np.percentile(channels.get("edge_score_geom", channels["edge_score"])[channels["hit_count"] > 0], 95)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "return_mix_score_raw_p95": (float(np.percentile(channels.get("return_mix_score_raw", channels.get("return_mix_score", channels["edge_score"]))[channels["hit_count"] > 0], 95)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "return_mix_score_p95": (float(np.percentile(channels.get("return_mix_score", channels["edge_score"])[channels["hit_count"] > 0], 95)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "return_depth_spread_p95": (float(np.percentile(channels.get("return_depth_spread", channels["edge_score"])[channels["hit_count"] > 0], 95)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "return_depth_spread_excess_p95": (float(np.percentile(channels.get("return_depth_spread_excess", channels.get("return_depth_spread", channels["edge_score"]))[channels["hit_count"] > 0], 95)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "beam_split_score_p95": (float(np.percentile(channels.get("beam_split_score", channels["edge_score"])[channels["hit_count"] > 0], 95)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "beam_mode_gap_p95": (float(np.percentile(channels.get("beam_mode_gap", channels["edge_score"])[channels["hit_count"] > 0], 95)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "beam_coherence_p50": (float(np.percentile(channels.get("beam_coherence", channels["edge_score"])[channels["hit_count"] > 0], 50)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "beam_coherence_p95": (float(np.percentile(channels.get("beam_coherence", channels["edge_score"])[channels["hit_count"] > 0], 95)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "return_coverage_p50": (float(np.percentile(channels.get("return_coverage", channels["edge_score"])[channels["hit_count"] > 0], 50)) if channels is not None and np.any(channels["hit_count"] > 0) else None),
        "edge_confidence_max": (float(np.max(channels.get("edge_confidence", np.zeros_like(channels["edge_score"])))) if channels is not None else None),
        "beam_profile": cfg.get("beam_profile", "gaussian"),
        "beam_width": cfg.get("beam_width", 0.35),
        "min_ray_weight": cfg.get("min_ray_weight", 0.05),
        "return_mix_gain": cfg.get("return_mix_gain", 1.0),
        "return_q_near": cfg.get("return_q_near", 15.0),
        "return_q_far": cfg.get("return_q_far", 85.0),
        "return_expected_spread_rel": cfg.get("return_expected_spread_rel", 0.01),
        "return_expected_spread_grad": cfg.get("return_expected_spread_grad", 0.10),
        "min_return_coverage": cfg.get("min_return_coverage", 0.25),
        "min_return_hit_count": cfg.get("min_return_hit_count", 4),
        "beam_split_min_weight_frac": cfg.get("beam_split_min_weight_frac", 0.10),
        "beam_split_min_abs_gap": cfg.get("beam_split_min_abs_gap", 0.10),
        "beam_split_confidence_gain": cfg.get("beam_split_confidence_gain", 0.50),
        "beam_coherence_mix_gain": cfg.get("beam_coherence_mix_gain", 6.0),
        "beam_coherence_split_gain": cfg.get("beam_coherence_split_gain", 0.80),
        "beam_split_min": cfg.get("beam_split_min", 0.22),
        "beam_split_front_min": cfg.get("beam_split_front_min", 0.10),
        "beam_split_back_min": cfg.get("beam_split_back_min", 0.07),
        "beam_coherence_solid_min": cfg.get("beam_coherence_solid_min", 0.45),
        "partial_occluder_gap_min": cfg.get("partial_occluder_gap_min", 0.20),
        "partial_occluder_gap_ratio_min": cfg.get("partial_occluder_gap_ratio_min", 0.015),
        "partial_occluder_back_ratio_min": cfg.get("partial_occluder_back_ratio_min", 0.12),
        "partial_occluder_excess_min": cfg.get("partial_occluder_excess_min", 0.05),
        "partial_occluder_coherence_max": cfg.get("partial_occluder_coherence_max", 0.82),
        "coherent_surface_guard": cfg.get("coherent_surface_guard", 0.92),
        "classification_priority": cfg.get("classification_priority", "sensor"),
        ...
        "classification_style": cfg.get("classification_style", "default"),
        "classifier_kwargs": _classifier_kwargs,
        "warnings": warnings,
        "auto_frame": frame_diagnostics,
        "depth_stats": stats,
        "classification_counts": counts,
        "autoframe_runtime_seconds": round(float(autoframe_runtime_seconds), 3),
        "render_runtime_seconds": round(float(render_runtime_seconds), 3),
        "report_runtime_seconds": round(float(report_runtime_seconds), 3),
        "pre_save_runtime_seconds": round(float(_time.time() - total_t0), 3),
        "save_runtime_seconds": None,
        "total_runtime_seconds": round(float(_time.time() - total_t0), 3),
        # Backwards-compatible alias.  Prefer total_runtime_seconds in new code.
        "runtime_seconds": round(float(_time.time() - total_t0), 3),
    }
    ...
    if out_dir is not None:
        ...
        if channels is not None:
            ...
            try:
                label_arr, _, _ = classified
                edge_path = _os.path.join(out_dir, base + "_geom_edge_mask.png")
                overlay_path = _os.path.join(out_dir, base + "_geom_edge_overlay.png")
                core_path = _os.path.join(out_dir, base + "_material_core.png")
                filled_path = _os.path.join(out_dir, base + "_material_filled.png")
                render_geom_edge_mask(label_arr).save(edge_path)
                render_geom_edge_overlay(label_arr, render_burst(last_burst, mode="shaded", bg="#08101a")).save(overlay_path)
                label_array_to_image(material_core_label_array(label_arr), style=cfg.get("classification_style", "default")).save(core_path)
                filled_arr, _ = fill_geom_edges_from_neighbors(
                    label_arr,
                    radius=int(cfg.get("edge_fill_radius", 1)),
                    min_fraction=float(cfg.get("edge_fill_min_fraction", 0.65)),
                    fine_priority=bool(cfg.get("edge_fill_fine_priority", False)),
                )
                label_array_to_image(filled_arr, style=cfg.get("classification_style", "default")).save(filled_path)
                paths["geom_edge_mask"] = edge_path
                paths["geom_edge_overlay"] = overlay_path
                paths["material_core"] = core_path
                paths["material_filled"] = filled_path
            except Exception as _e:
                paths["derived_output_error"] = repr(_e)
        diagnostics["save_runtime_seconds"] = round(float(_time.time() - _save_t0), 3)
        diagnostics["total_runtime_seconds"] = round(float(_time.time() - total_t0), 3)
        diagnostics["runtime_seconds"] = diagnostics["total_runtime_seconds"]
    ...
    return {
        ...
        "material_report": _material_report,
        "material_label_agreement": _material_label_agreement,
        "material_core_agreement": _material_core_agreement,
        "material_filled_agreement": _material_filled_agreement,
        "boundary_adjacency": _boundary_adjacency,
        "structure_density": _structure_density,
        "paths": paths,
    }
...
if __name__ == "__main__":
    ...
    if any(a in ("-h", "--help") for a in args):
        print("Usage:")
        print("  python lidar_lenses_wave.py --preset=indoor_structure [--material-board] [--out=DIR]")
        print("  python lidar_lenses_wave.py --preset=material_scan --material-board")
        print("  python lidar_lenses_wave.py --pack [--material-board] [--out=DIR]")
        print("")
        print("Presets:", ", ".join(sorted(SENSOR_PRESETS.keys())))
        print("Scenes:")
        print("  default         built-in cabin demo")
        print("  --material-board  dedicated material target board")
        print("")
        print("Options:")
        print("  --preset=NAME   run one sensor preset")
        print("  --pack          run overhead_layout + indoor_structure + material_scan")
        print("  --out=DIR       output directory (default: preset_out)")
    ...
    preset_arg = None
    pack = "--pack" in args
    material_board = "--material-board" in args
    ...
    for a in args:
        if a.startswith("--out="):
            ...
            preset_arg = a.split("=", 1)[1]

    prims = _build_material_target_board_scene() if material_board else _build_demo_scene()
    scene_name = "material_targets" if material_board else "cabin_demo"

    if pack:
        run_scene_preset_pack(
            prims, scene_name=scene_name,
            presets=["overhead_layout", "indoor_structure", "material_scan"],
            out_dir=out_dir, seed=seed,
        )
        print(f"saved preset pack → {out_dir}")
        sys.exit(0)

    if preset_arg is None:
        preset_arg = "material_scan" if material_board else "indoor_structure"

    result = run_sensor_preset(
        prims, preset_name=preset_arg, scene_name=scene_name,
        out_dir=out_dir, seed=seed,
    )
    print(f"saved → {result['paths'].get('contact_sheet', out_dir)}")
