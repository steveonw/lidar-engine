"""
lidar_lenses_wave.py — standalone multimodal ray/wave sensor sandbox (v0.6.6)
...
CLASSIFICATION_STYLES = {
    ...
    "material_focus": {
        **CLASS_COLORS,
        ...
        "wood_material": (210, 140, 65),
        "stone_hard": (185, 185, 175),
        "soft_material": (255, 150, 95),
        "foliage": (55, 220, 85),
        "sky": (8, 14, 22),
    },
}
...
def _style_classification_image(label_img: Image.Image, style: str = "default") -> Image.Image:
    ...
    out = arr.copy()
    for name, old_color in CLASS_COLORS.items():
        new_color = palette.get(name, old_color)
...
def _build_material_target_board_scene() -> List[Primitive]:
    ...
    v0.6.6: The earlier notebook material_targets scene could under-expose
...
# ─────────────────────────────────────────────────────────────────────────
#  GEOM_EDGE AS FIRST-CLASS STRUCTURE / BOUNDARY LAYER (v0.6.6)
# ─────────────────────────────────────────────────────────────────────────
...
def _material_label_agreement_from_label_arr(label_arr: np.ndarray,
    ...
    for piece_type, mask in sorted(masks.items()):
        ...
        for lab in ["metal_or_glass", "wood_material", "stone_hard",
                    "soft_material", "foliage", "hard_smooth",
                    "solid_surface", "geom_edge", "uncertain", "sky"]:
...
def material_filled_agreement_report(burst: Burst,
                                     ...
                                     radius: int = 1,
                                     min_fraction: float = 0.65,
                                     fine_priority: bool = False) -> List[Dict]:
...
def structure_density_report(channels: Dict[str, np.ndarray],
                             classify_kwargs: Optional[Dict] = None,
                             min_component_size: int = 20) -> Dict:
...
def material_label_agreement_report(burst: Burst,
                                    ...
                                    expected_labels: Optional[Dict[str, str]] = None,
                                    min_pixels: int = 5) -> List[Dict]:
    """Report how classifier labels agree with known material piece_types.

    This is a board/test metric, not a rendering metric.  It avoids the
    whole-frame problem where floor/backdrop pixels dominate material_scan.
    """
    classify_kwargs = dict(classify_kwargs or {})
    expected_labels = dict(expected_labels or DEFAULT_MATERIAL_EXPECTED_LABELS)
    label_img, _ = classify_pixels(channels, **classify_kwargs)
    label_arr = classification_label_array(label_img)
    masks = dominant_piece_type_masks(burst, prims)
    rows = []
    for piece_type, mask in sorted(masks.items()):
        n = int(mask.sum())
        if n < int(min_pixels):
            continue
        labels_here = label_arr[mask]
        uniq, cnt = np.unique(labels_here, return_counts=True)
        order = np.argsort(cnt)[::-1]
        top_label = str(uniq[order[0]]) if len(order) else "none"
        top_frac = float(cnt[order[0]] / max(n, 1)) if len(order) else 0.0
        expected = expected_labels.get(str(piece_type), None)
        expected_frac = None
        if expected is not None:
            expected_frac = float(np.sum(labels_here == expected) / max(n, 1))
        row = {
            "piece_type": str(piece_type),
            "pixel_count": n,
            "expected_label": expected,
        ...
        for lab in ["metal_or_glass", "wood_material", "stone_hard",
                    "soft_material", "foliage", "hard_smooth",
                    "solid_surface", "geom_edge", "uncertain"]:
            row[lab + "_frac"] = float(np.sum(labels_here == lab) / max(n, 1))
        rows.append(row)
    return rows
...
def _render_panel(panel_name: str,
    ...
    if name == "classification":
        ...
        return render_classification_with_legend(labels, counts, style=cfg.get("classification_style", "default")), "classification"
    if name == "geom_edge_mask":
        label_arr, _, _ = classify_to_label_array(channels, cfg)
...
def run_sensor_preset(prims: List[Primitive],
    ...
    return {
        ...
        "material_label_agreement": material_label_agreement_report(last_burst, channels, prims, classifier_kwargs_from_config(cfg)) if channels is not None else [],
        "material_core_agreement": material_core_agreement_report(last_burst, channels, prims, classifier_kwargs_from_config(cfg)) if channels is not None else [],
        ...
        "boundary_adjacency": boundary_adjacency_report(channels, classifier_kwargs_from_config(cfg)) if channels is not None else [],
        "structure_density": structure_density_report(channels, classifier_kwargs_from_config(cfg)) if channels is not None else {},
        "paths": paths,
    }
