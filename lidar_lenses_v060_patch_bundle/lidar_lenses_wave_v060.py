"""
lidar_lenses_wave.py — standalone multimodal ray/wave sensor sandbox (v0.6.0)
...
Capabilities:
  - v0.6.0 tuned stable presets + explicit material profiles from sweep testing
...
# ─────────────────────────────────────────────────────────────────────────
# ─────────────────────────────────────────────────────────────────────────
#  MATERIAL PROFILES — explicit multimodal material response
#
#  v0.6.0 consolidates the sweep findings: geometry/shape-only heuristics were
#  good enough for edges and occluders, but material boards needed explicit
#  material profiles.  The profile table is intentionally heuristic rather than
#  physically calibrated: it gives the classifier a stable, explainable signal
#  for wood / metal / glass / cloth / stone / foliage / skin-like surfaces.
# ─────────────────────────────────────────────────────────────────────────
MATERIAL_PROFILES: Dict[str, Dict[str, float]] = {
    # common scene/layout materials
    "ground":    {"audible": 0.55, "ultra": 0.50, "pol": 0.25},
    "floor":     {"audible": 0.65, "ultra": 0.55, "pol": 0.25},
    "wall":      {"audible": 0.80, "ultra": 0.88, "pol": 0.45},
    "roof":      {"audible": 0.85, "ultra": 0.92, "pol": 0.55},
    "chimney":   {"audible": 0.85, "ultra": 0.92, "pol": 0.65},

    # hard/rigid materials
    "wood":      {"audible": 0.75, "ultra": 0.55, "pol": 0.35},
    "log":       {"audible": 0.85, "ultra": 0.65, "pol": 0.30},
    "trunk":     {"audible": 0.85, "ultra": 0.65, "pol": 0.30},
    "post":      {"audible": 0.85, "ultra": 0.65, "pol": 0.30},
    "fence":     {"audible": 0.78, "ultra": 0.65, "pol": 0.30},
    "stone":     {"audible": 0.88, "ultra": 0.90, "pol": 0.45},
    "rock":      {"audible": 0.85, "ultra": 0.92, "pol": 0.45},
    "metal":     {"audible": 0.92, "ultra": 0.96, "pol": 0.90},
    "brass":     {"audible": 0.90, "ultra": 0.95, "pol": 0.85},
    "glass":     {"audible": 0.75, "ultra": 0.90, "pol": 0.82},

    # soft / absorbent materials
    "cloth":     {"audible": 0.45, "ultra": 0.12, "pol": 0.15},
    "fabric":    {"audible": 0.45, "ultra": 0.12, "pol": 0.15},
    "curtain":   {"audible": 0.40, "ultra": 0.10, "pol": 0.12},
    "rug":       {"audible": 0.42, "ultra": 0.10, "pol": 0.12},
    "carpet":    {"audible": 0.42, "ultra": 0.10, "pol": 0.12},
    "leather":   {"audible": 0.50, "ultra": 0.20, "pol": 0.25},
    "rubber":    {"audible": 0.48, "ultra": 0.18, "pol": 0.18},
    "skin":      {"audible": 0.22, "ultra": 0.10, "pol": 0.18},
    "head":      {"audible": 0.22, "ultra": 0.10, "pol": 0.18},
    "torso":     {"audible": 0.25, "ultra": 0.12, "pol": 0.18},
    "leg":       {"audible": 0.25, "ultra": 0.12, "pol": 0.18},

    # porous / partial occluder materials
    "foliage":   {"audible": 0.08, "ultra": 0.03, "pol": 0.10},
    "leaf":      {"audible": 0.08, "ultra": 0.03, "pol": 0.10},
    "leaves":    {"audible": 0.08, "ultra": 0.03, "pol": 0.10},
    "canopy":    {"audible": 0.08, "ultra": 0.03, "pol": 0.08},
    "grass":     {"audible": 0.10, "ultra": 0.04, "pol": 0.10},
    "smoke":     {"audible": 0.05, "ultra": 0.02, "pol": 0.04},

    # saloon/interior aliases
    "bar":        {"audible": 0.85, "ultra": 0.65, "pol": 0.35},
    "bar_top":    {"audible": 0.85, "ultra": 0.65, "pol": 0.35},
    "table":      {"audible": 0.78, "ultra": 0.62, "pol": 0.35},
    "table_leg":  {"audible": 0.78, "ultra": 0.62, "pol": 0.35},
    "chair":      {"audible": 0.78, "ultra": 0.62, "pol": 0.35},
    "chair_back": {"audible": 0.78, "ultra": 0.62, "pol": 0.35},
    "stool":      {"audible": 0.78, "ultra": 0.62, "pol": 0.35},
    "stool_leg":  {"audible": 0.78, "ultra": 0.62, "pol": 0.35},
    "target":     {"audible": 0.70, "ultra": 0.70, "pol": 0.40},
}


def _material_profile(prim: Primitive) -> Optional[Dict[str, float]]:
    """Return a profile for primitive piece_type if known.

    Matching is permissive: exact piece_type first, then lowercase tokens split
    on common separators.  This helps external scene builders use names like
    'dark_wood_chair' or 'cloth_rug' without writing custom impedance functions.
    """
    pt = str(getattr(prim, "piece_type", "") or "").lower()
    if pt in MATERIAL_PROFILES:
        return MATERIAL_PROFILES[pt]
    for token in pt.replace("-", "_").replace(" ", "_").split("_"):
        if token in MATERIAL_PROFILES:
            return MATERIAL_PROFILES[token]
    return None
...
def acoustic_impedance(prim: Primitive) -> float:
    ...
    v0.6.0: prefer explicit material profiles keyed by piece_type, then fall
    back to the original shape/color heuristic.  Override in user code by
    passing `impedance_fn=your_function` to compute_wave_channels.
    """
    prof = _material_profile(prim)
    if prof is not None:
        return float(prof["audible"])
...
def acoustic_impedance_ultrasonic(prim: Primitive) -> float:
    ...
    Explicit material profiles now separate cloth/fabric/leather from rigid
    boxes, which fixes the material-board failure mode found in the sweeps.
    """
    prof = _material_profile(prim)
    if prof is not None:
        return float(prof["ultra"])
...
def polarization_preservation(prim: Primitive) -> float:
    ...
    v0.6.0: keyed profiles let metal/glass read high, cloth/foliage low, and
    wood/stone intermediate.
    """
    prof = _material_profile(prim)
    if prof is not None:
        return float(prof["pol"])
...
def classify_pixels(channels: Dict[str, np.ndarray],
    ...
    Material priority:
        If material_priority=True, soft_material / hard_smooth labels win over
        partial_occluder and geom_edge.  This is intended for material_scan
        boards where material identity is more important than boundary shape.
    ...
    # v0.6.0: adaptive anti-edge threshold.
    ...
    # Ultrasonic-only categories
    if U_i is not None and A_tex is not None:
        hard_smooth = has_hit & (U_i > hard_smooth_ult_min) & ~foliage
        # SOFT_MATERIAL: lopsided audible-vs-ultrasonic response.  v0.6.0
        # removes the old upper cap at foliage_texture_min: cloth/leather can
        # legitimately have texture > 0.70, while foliage is already captured
        # by the low-acoustic-intensity rule above.  The ultrasonic ceiling
        # keeps wood/stone/metal from being swept into soft_material.
        soft_material = (has_hit & ~foliage &
                          (A_tex > soft_material_texture_min) &
                          (A_i > soft_material_intensity_min) &
                          (U_i < soft_material_ultrasonic_max))
    else:
    ...
    #   partial_occluder → geom_edge → foliage → hard_smooth →
    #   soft_material → optical_only → acoustic_only → solid_surface
    ...
    for name, mask in priority_order:
        m = mask & ~assigned
        final[name] = m
        assigned |= m
...
def _run_demo(quality: str = "standard",
    ...
    if classify:
        labels, counts = classify_pixels(
            ...
            adaptive_edge_percentile=None,
            hard_smooth_ult_min=0.85,
            soft_material_ultrasonic_max=0.55,
            attenuation_per_meter=attenuation_per_meter,
        )
...
# ═════════════════════════════════════════════════════════════════════════
# v0.6.0 SENSOR PRESETS — consistent scene test packs
# ─────────────────────────────────────────────────────────────────────────
...
SENSOR_PRESETS: Dict[str, Dict] = {
    ...
    "material_scan": {
        ...
        "adaptive_edge_percentile": 94.0,
        "hard_smooth_ult_min": 0.85,
        "soft_material_texture_min": 0.60,
        "soft_material_intensity_min": 0.15,
        "soft_material_ultrasonic_max": 0.55,
        "material_priority": True,
        "include_ultrasonic": True,
...
def scene_bounds(prims: List[Primitive]) -> Dict[str, np.ndarray]:
    ...
    v0.6.0 note: this uses per-axis extents instead of max-half-extent as a
...
def _render_panel(panel_name: str,
    ...
    if name == "classification":
        labels, counts = classify_pixels(
            channels,
            edge_anti_min=float(cfg.get("edge_anti_min", 0.30)),
            adaptive_edge_percentile=cfg.get("adaptive_edge_percentile", 90.0),
            hard_smooth_ult_min=float(cfg.get("hard_smooth_ult_min", 0.85)),
            soft_material_texture_min=float(cfg.get("soft_material_texture_min", 0.60)),
            soft_material_intensity_min=float(cfg.get("soft_material_intensity_min", 0.15)),
            soft_material_ultrasonic_max=float(cfg.get("soft_material_ultrasonic_max", 0.55)),
            foliage_intensity_max=float(cfg.get("foliage_intensity_max", 0.25)),
            foliage_texture_min=float(cfg.get("foliage_texture_min", 0.70)),
            partial_occluder_var_min=float(cfg.get("partial_occluder_var_min", 0.5)),
            partial_occluder_intensity_max=float(cfg.get("partial_occluder_intensity_max", 0.55)),
            material_priority=bool(cfg.get("material_priority", False)),
...
def run_sensor_preset(prims: List[Primitive],
    ...
    diagnostics = {
        ...
        "hard_smooth_ult_min": cfg.get("hard_smooth_ult_min", None),
        "soft_material_texture_min": cfg.get("soft_material_texture_min", None),
        "soft_material_ultrasonic_max": cfg.get("soft_material_ultrasonic_max", None),
        "material_priority": cfg.get("material_priority", None),
