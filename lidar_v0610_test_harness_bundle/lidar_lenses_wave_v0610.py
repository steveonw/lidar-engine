"""
lidar_lenses_wave.py — standalone multimodal ray/wave sensor sandbox (v0.6.10)
...
Capabilities:
  - v0.6.10 stability: preset-specific contact sheets, runtime split diagnostics, and edge-case contracts
...
def compute_wave_channels(burst: Burst,
    ...
    Args:
        ...
        carrier_mode: "x" for the legacy x-only carrier, "ensemble" for
            median-combined rotated carriers. v0.6.10 defaults the ensemble
    ...
    if carrier_mode == "x":
        ...
        if carrier_angles is None:
            # v0.6.10: golden-angle four-basis ensemble.  The older cross
    ...
    # exist.  v0.6.10 keeps raw anti-wave evidence separate from the geometry-
...
def edge_score_from_channels(channels: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    v0.6.10 prefers `channels["edge_score"]`, which may be the geometry-fused
...
def effective_edge_threshold_from_channels(channels: Dict[str, np.ndarray],
    ...
    v0.6.10 adds an upper clamp (`edge_anti_max`, default 0.95).  Without
...
def classify_pixels(channels: Dict[str, np.ndarray],
    ...
    Adaptive edge threshold:
        ...
        adaptive_edge_percentile) over hit pixels).  In v0.6.10 `edge_score`
        is geometry-fused by default, so scene-wide carrier residue is less
        likely to become geom_edge.
    """
    ...
    # v0.6.10: use the shared continuous edge score helper so binary geom_edge
...
def _build_material_target_board_scene() -> List[Primitive]:
    ...
    v0.6.10: The earlier notebook material_targets scene could under-expose
...
# ─────────────────────────────────────────────────────────────────────────
#  GEOM_EDGE AS FIRST-CLASS STRUCTURE / BOUNDARY LAYER (v0.6.10)
# ─────────────────────────────────────────────────────────────────────────
